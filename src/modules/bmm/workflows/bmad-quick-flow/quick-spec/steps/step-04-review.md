...
- IF D: Present strategic insights (Section 5), then exit workflow (Section 6)
- IF P: Read fully and follow: `{party_mode_exec}` with current spec content, process collaborative insights, ask user "Accept changes? (y/n)", if yes update spec then redisplay menu, if no keep original then redisplay menu
...
1. **Invoke Adversarial Review Task**:
   ...
   > The task should: review `{finalFile}` and return a list of findings. 2. **Process Findings**:
...
**Before exiting, surface system-level observations from your investigation.** During Steps 1-2, you read code, traced data flows, and understood integration points. That context is perishable — capture the high-value observations now.

Think beyond the spec you just wrote:

- **Resource & cost:** Did you encounter rate limits, API quotas, or plan tiers that the current usage pattern is approaching or exceeding?
- **Operational efficiency:** Are there processes (sweeps, syncs, enrichments) that run without pre-flight checks and waste cycles when resources are unavailable?
- **Hidden multipliers:** Does the area you investigated have secondary costs (cache misses cascading into API calls, N+1 patterns against external services)?
- **Architecture friction:** Did the investigation reveal coupling, missing abstractions, or type mismatches that would make future changes harder?
- **Data pipeline gaps:** Is there data being collected but not surfaced, or surfaced but not acted on?

**Present insights to the user after the spec is finalized:**

```
**Strategic Insights from Investigation:**

{Numbered list, each with:}
{1. Observation — what you noticed (name the service, endpoint, config)}
{2. Impact — why it matters (cost, reliability, velocity)}
{3. Suggested action — what to do, with rough effort estimate}

{Or "None — the investigation was narrowly scoped with no system-level observations."}
```

**These insights are NOT part of the spec itself** — they are separate observations for the user to consider independently. Present them after the spec completion message, before the next-steps menu.
...
- [ ] Strategic & operational insights presented to user on exit (even if "None").
