...
2. Run `~/bmad-method-v6/sync-bmad-workflows.sh` — syncs workflows, delivers the guard scripts from `custom/hooks/` into `<project>/.claude/hooks/`, and wires them **by script reference into the project's TRACKED `.claude/settings.json`** (2026-09-21; previously inline shell into the gitignored `settings.local.json`). Permissions and the MCP auto-enable flag stay in `settings.local.json`, which is where per-machine trust belongs. Fork-owned hooks are demoted out of the local file in the same act, because Claude Code concatenates hook lists across the two files and a hook left in both fires twice. A project whose `.gitignore` would swallow the tracked file gets the fork's block (owned by `tools/gitignore_claude_block.py`): it re-includes ONLY `.claude/settings.json` and `.claude/hooks/` — never the whole folder — is proven in a scratch repo and re-verified, and is re-rendered every run so the broad `!.claude/` form an earlier sync wrote is repaired (WF-20260925-092); otherwise the sync falls back and says exactly what to add. A release commits every path it owns in the target — replica, skills, bmad commands, guard scripts, hook wiring, the block — and nothing else (WF-20260925-091, `release_commit_set` in `tools/bmad-release.py`); a merged file that already held someone's edits is held and reported, never swept in. Full reasoning → `docs/hooks-registry.md` § *Where the fork wires a PROJECT hook*
