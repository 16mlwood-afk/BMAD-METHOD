...
**Relationship to its siblings.** `escalation-on-class-change.md` (STD-ESCALATE-001) fires **mid-flow**, when work already underway outgrows its lane. This fires **at intake**, when the ask itself is a scope extension. Same spirit (name the lane, propose the gateway, proceed unless vetoed), different trigger.

`scope-register-routing.md` (STD-SCOPEREG-001) is the **write-side** counterpart: this standard routes the *ask* and mandates a 4-part answer; that one governs the *row* the routed work leaves behind, so the lane named here becomes a checkable `route` field with a required `next_artifact` instead of evaporating into prose. **Naming the lane in an answer is not the same as the scope being routed** — a correctly-routed answer that leaves no routed row still produces registered-but-inert scope. When a §2 lane and a STD-SCOPEREG-001 route appear to disagree, this standard is the source of truth for the lane; that one governs how it is recorded (lane map: design-elevation → `R3-design` · quick-spec materiality → `R2-bounded-local` · capability → `R1-capability`; plus two routes this standard has no lane for — `R4-operational-milestone` and `R5-parked`, because an intake router only ever sees asks that *are* scope extensions).
