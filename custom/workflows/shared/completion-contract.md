...
- **Template coverage is DETERMINISTICALLY checked — the gate is now ARMED.** `tools/check-completion-disposition.js` (npm `check:completion`) walks `custom/workflows/` and flags a file that adopts the terminal-message shape (references STD-CLOSEOUT-001) but does NOT also reference this contract (STD-COMPLETION-001) — i.e. a close-out step that declares its message shape but not its completion disposition. **`check:completion -- --strict` runs in `npm test` and the pre-commit fast-path** (alongside `validate:close-out`): a likely gap now **exits 1 and blocks the commit**. The escape hatch is the logged, in-file override — a delivering close-out adopts the contract by referencing STD-COMPLETION-001 (including `advisory` with a why for a genuine no-deliver case), which exempts it. Default `check:completion` (no `--strict`) stays warn-only (exit 0) for soak-style visibility.
  - **Arming provenance (honest):** the gate was armed at owner direction **ahead of** the warn-then-gate soak default — the static scan was quiet (0 likely gaps) at arming, which is the *precondition* but not an elapsed real-run soak. The risk this front-runs: the heuristic's precision against workflows authored later by other sessions is unproven, so a future delivering-but-advisory close-out could false-block until its author adds the one-line contract reference (`advisory`). Mitigation: that escape hatch is cheap and self-documenting, the arming is fully reversible (remove `&& npm run check:completion -- --strict` from `test` + `.githooks/pre-commit`), and `validate:close-out` already establishes the same shape of gate. If false-blocks appear, widen the advisory-classification heuristic or revert to warn-only.
