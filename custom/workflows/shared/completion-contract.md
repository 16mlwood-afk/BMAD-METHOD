...
- **Template coverage is DETERMINISTICALLY checkable — and is shipping WARN-ONLY first.** `tools/check-completion-disposition.js` (npm `check:completion`) walks `custom/workflows/` and **warns** when a file that adopts the terminal-message shape (references STD-CLOSEOUT-001) does NOT also reference this contract (STD-COMPLETION-001) — i.e. a close-out step that declares its message shape but not its completion disposition. It is **warn-only by design** (exit 0 always, NOT in the pre-commit gate or `npm test`) during the soak phase, so its precision can be observed before it is trusted to block. Promotion to a hard gate (added to `npm test` / pre-commit, exit 1) is **Phase 2**, deliberately deferred until the warn output is proven quiet — the warn-then-gate composition pattern.
