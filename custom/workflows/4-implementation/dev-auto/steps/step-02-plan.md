...
- **Decision autonomy, not intent autonomy (safety layer rule 2).** Planning resolves *how* — file choice, task ordering, patterns. It does NOT backfill a *what* the grounding gate could not pin down. If investigation surfaces an intent ambiguity (two genuinely different shippable readings), that is an intent gap → HALT per instruction 5. Do not pick the reading you find most plausible.
...
3. Read `{project-root}/_bmad/bmm/workflows/4-implementation/dev-auto/templates/spec-template.md` fully. Fill it out based on the intent and investigation. If `{preserved_intent_contract}` is non-empty, substitute it for the `<intent-contract>` block in your filled spec before writing. Write the result to `{spec_file}`.
...
Re-read the workflow file (`{project-root}/_bmad/bmm/workflows/4-implementation/dev-auto/workflow.md`), then re-read `{spec_file}` from disk and verify the spec meets the READY FOR DEVELOPMENT standard.
...
Read fully and follow `{project-root}/_bmad/bmm/workflows/4-implementation/dev-auto/steps/step-03-implement.md`
