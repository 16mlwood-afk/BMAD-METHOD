...
<workflow>
  ...
  <step n="10" goal="Completion communication and user support">
    ...
    <action>Declare a `completion_disposition` per `shared/completion-contract.md` (STD-COMPLETION-001) as a one-line tail on the close-out: `pr_merged` / `pr_open` with the PR when the story shipped, or `owner_gated_residue` naming each remaining blocker and why it is owner-gated (a credential, a prod-data mutation, a destructive migration, a scope re-plan). dev-story is a completion workflow — moving the story to "review" with delivered code is `pr_open`/`pr_merged`; stopping at "here is what I would do" with no disposition is the invalid commentator exit (contract §3). `advisory` applies ONLY if the owner scoped this run to analysis.</action>
