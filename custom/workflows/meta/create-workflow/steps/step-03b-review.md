...
- **Persona placement** (`shared/persona-placement.md`, STD-PERSONA-002: personas are for human-facing judgment, not plumbing) — a mechanical / rails / sync / internal-sub-step workflow must NOT bake in or reference a named persona/voice (no `voice:`, no Rhea/Sol/Mara framing, no invented character). If the workflow IS genuinely human-facing and judgment-bearing and a named voice would help, that is `workflow-personas` (presentation) or `create-agent` (an invocable agent) — not something this workflow invents inline. A persona on plumbing is a finding.
