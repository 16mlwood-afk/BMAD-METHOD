...
- **Concrete → a focused Claude Design paste prompt.** If the surface and the visual/interaction change are clear (e.g. "the orders drawer shows the discrepancy but has no action to resolve it"), emit a paste-ready Claude Design enhancement prompt instead of a code spec: the connect-the-repo line, the component/route files to read, a *keep-everything-else-as-is* guard, the precise interaction to deepen, any sibling pattern to borrow, and the surface's `docs/design-policy.md` constraints. Reference the surface's design brief if one exists. This is the design analog of "direct quick-dev" — the signal already names the surface + the change. (If the surface is *settled* and the signal is "it should do more," prefer routing through `design-elevation`, which formalizes this prompt with provenance.)
