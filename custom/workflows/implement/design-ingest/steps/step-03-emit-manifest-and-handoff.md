...
- **Fingerprint scan (`fingerprint_scan:` block)** — this is the ingest-boundary arm of the canonical taxonomy in `design-standards.md` (§ AI Fingerprint Detection / § Enforcement coverage): run the shared scanner over every fetched frame source file before emitting —

  ```bash
  bash {project-root}/scripts/design-fingerprint-scan.sh <frame-html-files...>
  ```

  Record the result in the manifest frontmatter as `fingerprint_scan: { ran: true|false, deterministic_findings: <n>, findings: [{rule, file, construct}...], declared_exceptions: [...], advisory_note: "<the scanner's advisory line>" }`. **Rules:** (a) findings do NOT block the emit — ingest is non-destructive; they are stamped so the step-03 pause and every later resume can see the design arrived carrying prohibited constructs, and the apply phase must not silently reproduce them; (b) a clean scan is recorded as `deterministic_findings: 0`, never as "design compliant" — most of the taxonomy is advisory by construction; (c) if the scanner is absent (not yet synced to this project), record `ran: false` with the reason — an absent scan must be distinguishable from a clean one; (d) surface any findings prominently at the §4 pause walkthrough, next to the section inventory.
