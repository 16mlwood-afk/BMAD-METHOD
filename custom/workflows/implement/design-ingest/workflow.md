...
- `{manifest_path}` — Absolute path to the emitted ingest manifest on disk. (Paths RECORDED INSIDE the manifest are governed by the opposite rule — see `manifest-schema.md` → "Path invariant": anything a downstream workflow dereferences must be repo-relative, tracked, and durable, never a session scratchpad.)
- `{run_started_at}` — UTC timestamp stamped from the harness clock at the top of step-01. The denominator for the §5a concurrent-run check: an output artifact for this slug that is NEWER than this is proof of a parallel session, not a prior run.
- `{concurrency_verdict}` — `no-collision` | `prior-manifest (re-ingest)` | `concurrent-detected` | `unknown (failed open)`. Set by step-01 §5a before the fan-out; carried into the step-03 pause.
- `{surface_existence}` — `brownfield` | `net-new-surface` | `unknown (probe failed, failed open)`. Set by step-01 §5b before the fan-out, probing `origin/main` (never the working tree) for a route + page component for `{target_slug}`. Mirrors `design-implement`'s already-ratified existence gate so the EXPENSIVE producer stops where the CHEAP consumer already would. Carried into the manifest receipt (`ingest.surface_existence`) and the step-03 pause, where it is terminal for how the handoff presents itself.
...
- **Check for a concurrent run on `{target_slug}` BEFORE the fan-out — the spend is what's being protected.** `{target_slug}` is a canonical collision key: two sessions ingesting the same surface provably compute the same slug and the same output path. Step-01 §5a probes for a parallel run at the moment the slug resolves — one step before the most expensive phase in the workflow — and STOPS if it finds one. Detecting the duplicate at the step-03 manifest write is a failure even when the manifest is correct: by then a full multi-agent fan-out is already spent and the other session's staged work is one clobbering write away (observed 2026-07-20, cash-recovery — two ~1M-token fan-outs for one artifact, caught only by an accidental Write-tool refusal). The check **fails OPEN** on any ambiguity and never writes to the register: unreadable is UNKNOWN, not clear, and a claim an agent writes for itself is exactly the unattributable claim the register cannot honour.
- **Probe for a surface to implement against BEFORE the fan-out — gate the producer, not only the consumer.** `design-implement` already soft-exits when the target surface has no route and no page component. Running that probe ONLY in the cheap downstream consumer inverts the cost curve: this workflow is routed the largest surfaces by design, so the spend the gate exists to avert is paid in full before the gate is reached. Step-01 §5b runs the same probe here — `ls`-class, against `origin/main` (a stale checkout reports a false absence, which is the direction that fires it), verdict decided by route + page component alone (a backing object is not a surface). Soft, like the consumer's: ingest is the tolerant half and never hard-refuses. **And the verdict is TERMINAL for presentation** — a manifest built over a net-new surface may be handed off as a catalogue, never as ready to implement. The 2026-07-31 `intake-pilot-console` run had already computed the fact, written it into a flag, and handed off READY anyway; a determination the artifact records but the handoff contradicts is not a gate. (`FG-2026-07-28-07`.)
