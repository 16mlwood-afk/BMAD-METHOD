...
| 6 | Prior-filing reconcile (not a blind block) | A receipt at `{implementation_artifacts}/filing-receipts/de-vat-{period}-receipt.md` or a DB `FILED` is **evidence of an attempt, not proof the portal holds the filing** — it can be a flaked submit (see step-05 portal-truth verification). Do NOT hard-block on it. Reconcile against portal-truth: read-only `mcp__avask-filing__avask_reconcile_filing` / `avask_reconcile_portal` **+** the human portal-truth marker (`scripts/set-filing-status.sh` state, the canonical "did it file?"). Branch: **portal CONFIRMS** (row left `Provide Data`, or marker `filed`) → BLOCKED — genuinely filed, re-filing double-files. **portal CONTRADICTS** (row still `Provide Data`) → soft-flag + DISCLOSE ("receipt says filed, portal row still Provide Data — evidence of an attempt that didn't land; re-filing is correct — confirm you didn't file manually") and PASS. **reconcile ambiguous/unavailable** → default-stop BLOCKED asking the owner to confirm portal-truth by hand (never proceed into a possible double-file on an unknown). A pre-existing `preflight-{period}.json` alone (interrupted session, no receipt) → note it, not a block. |
