...
- **If the user referenced a UI table/grid (by name, screenshot, or description):** locate the actual data source first — the API route, server action, or database query that populates it. Read the real data and column definitions rather than relying on visual descriptions or screenshots. The source code is the ground truth; the rendered UI is a lossy projection of it.
- **If the user attached a visual/layout bug screenshot:** follow the step-01 §3b digestion process — identify the component, read the full render tree, and produce the visual digest with current values and proposed changes before proceeding to execution.
...
- **y:** "**NEXT:** Read fully and follow: `{project-root}/_bmad/bmm/workflows/implement/quick-dev/steps/step-03-execute.md`"
