...
- `{bundle_conformance}` — `pass` | `UNVERIFIED(<reason>)` | `halted(truth: <ids>)` | `pass-with-notes(<n>)`. Set in step-01 §SHARED.1b under the binding split (`shared/brief-binding-contract.md`, 2026-09-19): the run HALTS only when the design breaks one of the brief's Part 2 truth tests or fails T0, the five-second answer test. A suggested frame not drawn, different chrome, or a different composition is an advisory note (`pass-with-notes`), disclosed in SHARED.2 and carried to the §9 report — never a halt. Complements step-03 §2f (impl-side frame coverage — build fidelity to what the designer DREW, unchanged) and §2d (impl-side shell check, unchanged).
...
- `{visible_delta}` — What a person looking at the page will NOTICE when this lands, in the words they would notice it in: `{ visible, unchanged, composition: design|existing|mixture, composition_note }`. **`visible: "nothing you will notice"` is a legitimate and important value, not a gap.** Emitted and SURFACED TO THE USER in step-02b §4f — before the grid, before any code moves — and it leads block 1 of the §9 close-out. Unconditional: every run, no `n/a`. Owner instruction 2026-09-26; a run whose reports were all code-level left the owner to discover by opening the page that nothing had visibly changed.
- `{departures}` — Places where this run put something of ITS OWN in the design's place, rather than leaving the design's item unbuilt. Each `{ id, row, kind: treatment|copy|composition|column|policy-conflict, design_says, we_did, interim: bool, why, ask }` — `ask` is the §4 ask number in `{sendback_artifact}` that answers for it. Accumulated in step-04 §5c as the apply walks, NOT inferred at the end: a departure is recognisable at the moment it is taken and reconstructible only with difficulty afterwards. A capability disposition already ruled at step-02b §4, a §5b forced-and-logged path substitution, and a faithfully applied row are all NOT departures (workflow.md Critical Rule "Claude Design is the source of truth for design", carve-outs a–c).
- `{sendback_artifact}` — `none` when `{departures}` is empty, else the path of the emitted `SENDBACK-<target_slug>-v<next>.md`. Every departure-class ledger row cites it by `sendback:<file>#ask-N`; a reason alone is not a legal disposition for a departure. Contract: `sendback-template.md`. Checked by `node ~/bmad-method-v6/tools/check-design-sendback.js`.
...
- **SAY WHAT THE OWNER WILL SEE — before you build it, and first when you hand back. "Nothing you will notice" is the most valuable sentence this workflow can produce.** Owner instruction, 2026-09-26, verbatim:
  ...
  **Every report was ACCURATE and none of them answered the question.** A forked token layer removed, eight tinted chips retired, the centre workspace rebuilt — all true, all verifiable, all about the CODE, and all reading as progress towards the bundle he supplied. The one thing nobody said was *you will not notice this*. He found out by opening the page, and asked twice. **So a code-delta report is not a substitute for a visible-delta report, however exhaustive the ledger is.**

  Two required outputs and one framing rule:

  1. **BEFORE the apply — `{visible_delta}`, surfaced to the user** (`steps/step-02b-regression-surface.md` §4f). Region by region, in a person's words; what will still look like the old page; and which composition the surface will have. **Unconditional — every run, no `n/a`, and autonomous mode suppresses asking, not saying.** A declaration, never a gate: it does not halt and does not wait for input. Its job is that the owner *can* stop it from one sentence, in time.
  2. **THE HAND-BACK LEADS WITH IT** (`steps/step-04-apply-and-deliver.md` §9 / the OUTPUT CONTRACT below). Whatever else the report holds, its first content is what a person looking at the page would notice. Tokens, gates, retired rules, test counts and row dispositions come **after**. **A run that changed nothing visible says so first.**
  3. **A COMPOSITION GAP IS A DIFFERENT THING, NOT A PARTIAL ONE.** Implementing the bundle's CONTENT into an UNCHANGED COMPOSITION is a **material difference from the bundle the owner supplied** and is stated as such, up front, in those terms — **never as "pass 1 of N"** or as progress towards the design. A whole-page design received as content in the old layout is not a partial version of that design. "Pass 1 of 2" tells a reader to expect convergence; *"the content of your design, in the existing layout, which this pass does not change"* lets him stop the run. The first framing is precisely why he could not.

  **Enforcement, honestly: PROBABILISTIC on all three.** No check can judge whether a sentence about visible change is true or useful, and a gate demanding one on every run would be satisfied with boilerplate inside a week. **The mechanisable half is named and deliberately NOT built** — the run already knows which regions it touched, so a required non-empty field naming a region is cheap; and an empty visible change **declared as such** is a legitimate value of it, never a failure, because that is the answer the rule most exists to obtain.

- **CLAUDE DESIGN IS THE SOURCE OF TRUTH FOR DESIGN. An implementer may not cherry-pick — a departure goes back as a formal SENDBACK, and the loop runs between the two of you.** Owner ruling, 2026-09-26, verbatim:
  ...
  **The failure this stops is NOT concealment, and reading it as one reproduces it.** The two runs that produced this ruling recorded every departure honestly and at length — *"v15 wins over the design, and it is logged"*, *"the implementation beats the design … Keeping ours"*, a table column dropped with the missing field named. Nothing was hidden. What was wrong is that **a well-documented unilateral decision was treated as a legitimate outcome**. So disclosure is not the fix: those ledgers already disclosed. The fix is the **return leg** — the departure becomes a question the designer answers, in an artefact, and the design resolves it.

  **The test, one sentence: did this run put something of OUR OWN in the design's place — a different value, label, column count, composition, or treatment? Then it is a DEPARTURE and it needs a sendback. Is the design's item simply not there yet, with nothing of ours standing in for it? Then it is NOT YET BUILT and it needs only an honest deferral.** A departure *asserts an alternative*; a deferral *asserts nothing*. That distinction is the whole mechanism, and it is why holding four items back correctly — the header, the undo panel, a `Proposal reasoning` row, a column no field records — costs a run nothing: none of them is a sendback item, and pushing them through one would make the rule expensive enough to be switched off.

  **`policy wins over the design` IS NOT A SELF-SERVICE VERDICT.** Where the design contradicts `docs/design-policy.md`, two authorities the owner set are in genuine conflict, and an implementer resolving it by picking the policy has taken the design decision anyway — just in the direction that feels safe. It goes to the designer as a sendback item. **The interim behaviour is still whatever avoids shipping a known policy hard failure** — do not ship the violation to make a point — but it is labelled **interim** in the ledger and in the sendback, never recorded as a win. The observed case: the design set twelve section labels `text-transform: uppercase`, which `docs/design-policy.md` §4 forbids and §5 lists as a hard failure; dropping the transform was the right *interim* act and the wrong *verdict*.

  **Three carve-outs, because a rule that fires on ordinary work gets switched off.** None of these is a departure: **(a)** a capability disposition already ruled at step-02b §4 — an `additive` strategy retaining something the design drops is an owner decision already taken, not a new one; **(b)** a **forced** copy/chrome deviation logged under §5b (the design links to a route that does not exist) — forced-and-logged is already the contract, and a sendback is owed only when the force is a *design* question rather than a missing path; **(c)** a row the run applied faithfully. Mechanism, dispositions, and the refusal: `steps/step-04-apply-and-deliver.md` §5c. Artefact contract: `sendback-template.md`.

  **Enforcement, split honestly.** **PROBABILISTIC** — whether a run *recognises* that it has departed is a judgement no check can make, and this prose plus §5c is that whole tier. **PROBABILISTIC-with-a-refusal** — §5c refuses to let the pass close while a departure-class row carries no sendback reference; that is stronger than prose but still inside the model's own execution, so do not call it enforced. **DETERMINISTIC in verdict, probabilistic in invocation** — `node ~/bmad-method-v6/tools/check-design-sendback.js` decides, mechanically, whether every departure-class ledger row cites a resolvable sendback and whether that sendback carries its seven sections; §5c and §6 both call it, and nothing makes a run call it. The tier that would close invocation is a **per-project CI check over the emitted ledger**, and it is **named here and deliberately not built**: it does not ship with the workflow sync, so authoring it in the fork would deploy nothing. Same honesty split the `design-handoff` step-01 §3f viewport note already states for itself.
...
- **A BUNDLE-DIFF CANNOT CERTIFY POLICY CONFORMANCE — so cede the dimension. That is a statement about the INSTRUMENT, never an authority ranking over the designer.** The Claude Design bundle AND its README are generated *from* the project's design policy (the README says so: "policy-first; foundations derived from the policy"), and they can diverge from it (a real bundle shipped a banned colored-glow `@keyframes` + no `prefers-reduced-motion`). So the bundle is the authoritative source for **treatment** — match its proposed pixels, that is the workflow's job — while **policy conformance is not measurable from a diff against it**: faithfully matching a bundle that breaks the policy scores `✓`. Therefore: read the one statically-checkable policy rule — **page-shell / layout intent — from `docs/design-policy.md`** (not the generated README), and **CEDE the rest of the policy contract — prohibitions / tone / motion / iconography — plus all interaction behavior** to the workflows that own the live evidence (`design-review`, `design-review-pr`, `verify`), via the step-03 §2e cede + step-04 §9 disclosure. Never invent a half-check (a grep for `rounded-full`, an `@keyframes` scan) and present it as conformance — a bundle-anchored check against a generated, divergent bundle lies; an honest cede does not.

  **This rule previously read "The bundle is a generated PROPOSAL; the spec is `docs/design-policy.md`", and that wording was the licence a real run used to decide a design question itself** — pass 1 of the mapping-queue cockpit cited *"the workflow's 'the bundle is a generated PROPOSAL; the spec is the policy' case"* to override the design's attention treatment, and headed the section *"v15 wins over the design"*. Reworded 2026-09-26 with the owner ruling above. **The split that was being collapsed:** the policy constrains what may **SHIP** — an interim behaviour never introduces a known policy hard failure, and that constraint is not loosened here. It does **not** settle who owns the **DESIGN DECISION** when a specific design item and a specific policy clause conflict; that returns to the designer as a `⊘ interim(policy-conflict: …)` row and a numbered sendback ask (`steps/step-04-apply-and-deliver.md` §5c, `steps/step-03-build-grid.md` §2e "The boundary"). **Neither side is recorded as having won until the designer answers.**
...
**BLOCK 1 OPENS ON `{visible_delta}` — what a person looking at the page would NOTICE — and nothing
precedes it.** Not the mode, not the delta count, not what was retired or rebuilt, not the merge.
Those are code-level facts and they go after, or in block 2. **A run that changed nothing visible
says so in its first sentence**, and a run that implemented the bundle's content into an unchanged
composition says *that*, in those terms, before anything else (Critical Rule "Say what the owner will
see"; step-02b §4f). The failure this ordering exists to stop is a wholly accurate report — a token
fork removed, eight chips retired, a workspace rebuilt — that reads as progress towards a design the
page does not resemble.
