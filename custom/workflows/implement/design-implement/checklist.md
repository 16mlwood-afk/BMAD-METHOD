...
- [ ] **`{visible_delta}` declared and SURFACED TO THE USER before the grid and before any code moved (§4f)** — unconditional, every run, no `n/a`. Region by region in a person's words; what will still look like the old page; and which composition the surface will have. **"NOTHING you will notice" was stated plainly where true** rather than omitted or dressed as progress. It did NOT halt or wait for input — a declaration, not a gate. Owner instruction 2026-09-26.
- [ ] **A composition gap was named as a MATERIAL DIFFERENCE from the supplied bundle (§4f)** — where the run implements the bundle's CONTENT into an UNCHANGED COMPOSITION, that was said up front, in those terms, and **never framed as "pass 1 of N"** or as progress towards the design.
...
- [ ] Frame-coverage rows emitted (§2f) — **the frame contract was loaded and one row emitted per frame.** Source in precedence order: brief §7 Surface Inventory; OR (raw-URL run, no brief) the bundle's declared `{design_frame_inventory}` (step-01 URL.3a — the drilled drawer + each §13 lookup); OR the manifest. Always emit the block; only if NO source yields a frame set, say so and mark it `needs human confirmation`. A frame drawn-but-unbuilt is Tier-1 (`FRAME MISSING in impl` — the verdict for the §13 lookup drawers on a no-brief URL run); a brief-suggested frame the bundle never drew is an advisory note (`SUGGESTED FRAME NOT DRAWN — designer's call`, counted in `{frame_uncovered_count}`), never inferred and never routed back as a defect (`shared/brief-binding-contract.md` §4).
- [ ] Bundle → brief conformance (step-01 §SHARED.1b) halted ONLY on a Part 2 truth test or T0 (the five-second answer); frame / chrome / composition differences were recorded as advisory notes, not halts. A grid that ran the component sweep only over frames that already exist in impl — never enumerating the contract — is non-conformant; "no brief" is not "no contract."
...
- [ ] Copy screen run on EVERY user-visible string before merge (§5b pass 4, STD-COPY-SCREEN-001) — `check-copy-screen.js --strings --strict` clean, parts (a)/(b)/(d) read by hand, strings added outside the Copy deck listed in §9, count screened and count changed reported
...
- [ ] **Departures classified and sent back (§5c)** — every place this pass put something of its OWN in the design's place (a different value, label, column count, composition or treatment) is disposed `⊘ departure(…)` / `⊘ interim(policy-conflict: …)` **citing `sendback:<file>#ask-N`**; a reason alone was NOT accepted. A design item left untouched with nothing of ours standing in for it stayed a cheap `⊘ deferred` — no sendback owed. The three carve-outs were honoured (a step-02b §4 capability disposition · a §5b forced path substitution · a faithful apply). **No verdict verb was written** — not `policy wins`, `v15 wins`, `the implementation beats the design`, or `keeping ours`. Owner ruling 2026-09-26; workflow.md Critical Rule *"Claude Design is the source of truth for design"*.
- [ ] **Sendback artefact emitted and reconciled (§5c)** — when `{departures}` is non-empty, `SENDBACK-<target_slug>-v<next>.md` exists per `sendback-template.md` with all seven sections in order, **each ask naming at least two acceptable shapes** (one shape is a specification wearing a question mark), frontmatter `departures:` equal to the rows citing it, every `#ask-N` resolving. Force-added and committed in §6, linked in the PR body and §9. Verified with `node ~/bmad-method-v6/tools/check-design-sendback.js --check <ledger>` **and** `--sendback <file>`, not by eye.
- [ ] **The §9 report OPENS on "What you will see"** — the visible delta in a person's words, before the mode, the row count, what was retired or rebuilt, or the merge. `⚠ THIS DOES NOT LOOK LIKE THE BUNDLE YOU SUPPLIED` present whenever the composition is not the design's. Any difference from the §4f pre-build declaration named. The three "say this first" rules ordered as declared: visible delta → already-applied attribution → checkpointed disposition, all ahead of any row count.
- [ ] **§9 "Departures sent back to the designer" section present — including the empty case** — "None — every design item was either applied faithfully or left explicitly NOT YET BUILT" is a required statement, because an omitted section and a clean one read identically and mean opposite things.
