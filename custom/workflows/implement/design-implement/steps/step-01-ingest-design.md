...
3. **TC1, TC2, TA1, TA2, TF1 — controls, attention and faults** (`{project-root}/_bmad/bmm/workflows/design/shared/controls-and-attention.md`, STD-CONTROLS-ATTENTION-001). These five are Part 2 tests like any other and halt the same way; they are called out here because they are on **every** brief and are judged against what the bundle DRAWS rather than against a data claim. Owner, 2026-09-20: *"drift between us thinking there's a live offer versus Amazon not receiving the offer is just a back end bug. Why is it classified as a common operational problem?"*
   - **TC1** — list every action control the bundle draws (not navigation, filter, sort, expand or open). For each, say what the press tells the system that the system does not already have. A control whose answer is *"nothing — the system could run this itself"* is a **HALT**. Cross-check the brief's §4f-c background-work list: a bundle that turned one of those into a per-row control has failed this test.
   - **TC2** — read every instruction in the bundle's prose. Each one needs a control on the same surface that performs it. An instruction with no mechanism is a **HALT**.
   - **TA1** — rank what the eye reaches first against what costs most if pressed or missed. A mismatch at the top is a **HALT**, and so is the most consequential control drawn as chrome (a go-live switch sitting in a filter row).
   - **TA2** — count where each of the bundle's three loudest facts is said at rest. The same fact twice is a **HALT**; a decomposition carrying a genuinely different fact is a pass.
   - **TF1** — for each category the bundle counts as work, ask who must act for it to stop recurring. An engineer, a rule or a producing system means it is a fault: it must be named as one, owned by someone other than the operator, and counted separately. A fault shown as workload, or a control that exists only to compensate for one, is a **HALT**.

   **Do not widen these into style halts.** A colour, radius, font-weight, spacing or wording difference answers neither question the standard asks and stays an advisory note, exactly as before — `controls-and-attention.md` §4 excludes those on purpose.
