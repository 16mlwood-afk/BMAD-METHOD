...
>
> **Recover by slug, in this order, and stop at the first that resolves:**
> 1. Another `.html` under the SAME directory as the missing target (a renamed/versioned wrapper — `templates/<slug>/<anything>.html`, preferring the newest-looking version suffix).
> 2. The **`ui_kits/<slug>/` modules** — derive `<slug>` from the target's parent directory. The frame's app entry (`*App.jsx` / the module that assigns `window.<Name>App`) is the root; its siblings are the frame set. Read these as the design source directly.
> 3. A `templates/<sibling-slug>/*.html` whose `<script src>` chain reaches `ui_kits/<slug>/` (another wrapper mounting the same kit).
>
> **SAY IT — in the §SHARED.2 summary and carried to the §9 report:** *"the named target `<path>` no longer exists in the design project; resolved the design from `<what worked>` instead."* An ingest that silently substitutes a different source is worse than one that 404s, because every later "matches the design" claim then refers to a source the owner never named. **Only refuse when NONE of the three resolve** — that is a genuinely absent design, not a moved one.
>
> **This is distinct from URL.6's near-empty-catalog guard.** That backstops a *plausible-but-thin* catalog (the bundle was read but yielded too little). This is a *hard 404 on the named entry point* — the earlier, more legible failure, and the one URL.6 would only ever catch downstream as a mysterious thin result.
