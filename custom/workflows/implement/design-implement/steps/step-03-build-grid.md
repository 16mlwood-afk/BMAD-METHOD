...
- **NET-NEW page — diff against the SIBLING BASELINE, never an empty impl.** When `{impl_page_shell}.source == "sibling-convention"` (step-02 §1a — a brand-new surface with no existing page to walk), the Implementation column IS that house-convention baseline (the sibling operational pages' cap + centering); **use it, and never let the impl side read as empty.** An empty impl value makes this row silently no-op (design `full-bleed` vs *nothing* → false green) — the exact hole through which a net-new operational page ships full-bleed against a capped house convention (the accounting-tools `/vat-filings` miss: full-bleed shipped while every sibling — `/avask` `max-width:1400px`, `/order-summaries` 1480px — caps + centers). This is the page-shell twin of §2f's "no brief is not no contract": no existing page is not no baseline. **And treat the bundle wrapper's `full-bleed` as a RENDERING ARTIFACT, not intent** — Claude Design renders the frame standalone with no app shell, so its root is `max-width:none` by construction. So a design resolving `full-bleed` while the sibling baseline caps is **Tier-1**: the net-new page adopts the **house cap**, UNLESS the `authoritative: true` (`docs/design-policy.md`) entry explicitly wants THIS surface full-width. The bundle's artifactual full-bleed can never, by itself, license a full-bleed net-new page against a capped house — the policy must say so. ("full-width **within the content container**" means *fill the capped container*, not bleed to the viewport.)
...
- **Frame IDENTITY on a handheld-first surface follows the operator-artifact contract** (`custom/workflows/design/shared/operator-artifact-contract.md` **B3**): a state variant is a *degraded state of the canonical frame*, so its name is the `{primary}--{state}` stem, its chrome / layout skeleton / primary-action position are unchanged, and exactly one region differs. Match state frames by that stem — a variant drawn with its own nav or hero is a **mis-modelled state**, not a new frame, and is a Tier-1 composition delta rather than a missing-frame row. **Scope cede, stated plainly:** this workflow compares the impl against the bundle, so it can check that the *stem and skeleton* hold; it CANNOT judge whether the surface satisfies **B7** (compressed operational stack vs **DASHBOARD OPENER**) — that is a judgment against the policy, not the bundle, and a hero-plus-chip-wall bundle reproduced faithfully scores `✓` here by construction. B7 belongs to `design-review-pr` **C5**. Do not fake it into a grid row; name it as ceded, same posture as §2e.

### 2e. Policy-conformance, prohibitions, and behavior — CEDED, not checked (a bundle-diff cannot certify policy conformance)

**This section is about EVIDENCE — what this workflow's one instrument can measure. It is NOT an authority ranking between the design and the policy, and it must never be read as one.** That misreading is exactly what happened: this section's former title — *"the bundle is a generated proposal, not the spec"* — was quoted almost verbatim by a real run to justify overriding a design treatment on its own authority (the 2026-09-26 owner ruling; see the boundary paragraph below).

The bundle and its README are **generated by Claude Design from `docs/design-policy.md`**, and they can themselves diverge from it (a real bundle shipped a colored-glow `@keyframes` pulse + no `prefers-reduced-motion`, both banned by the very policy it was scaffolded from). So design-implement, which diffs the impl against that bundle, **cannot certify policy conformance** — matching a bundle that breaks the policy faithfully reproduces the break and scores `✓`. That is a fact about the instrument, not about who is in charge. Three dimensions are therefore CEDED here (the same disclosure model as the content lane §2c), never faked into a grid check:
...
#### The boundary — this section cedes MEASUREMENT. It does not confer a DECISION.

**Two different things, and collapsing them is what produced the 2026-09-26 owner ruling** (*"claude design is the source of truth for design never do this again"*; `workflow.md` Critical Rule *"Claude Design is the source of truth for design"*):

| | Who owns it | Where it is settled |
|---|---|---|
| **Can the grid CERTIFY policy conformance from a bundle-diff?** | Nobody — the instrument cannot. | Ceded above, to `design-review` / `design-review-pr` / `verify`. Unchanged. |
| **When a SPECIFIC design item and a SPECIFIC policy clause actually conflict, who decides?** | **The designer**, on a question put to them. | `step-04` §5c — a `⊘ interim(policy-conflict: …)` row and a numbered sendback ask. |

**So a detected conflict is NOT a licence to override the design here.** What this section authorises is a *cede* — saying plainly that the dimension was not measured. It authorises nothing about a conflict it happens to notice on the way. Concretely, when the design specifies something a named policy clause forbids:

1. **Do not ship the known violation** — the interim behaviour complies with the policy. (The ship constraint is real and is not being loosened.)
2. **Do not record that as a verdict.** Disposition `⊘ interim(policy-conflict: <policy §> vs design) → sendback:<file>#ask-N`. **Never `policy wins`, `v15 wins`, `the implementation beats the design`, or `keeping ours`.**
3. **Return it to the designer** with the policy clause quoted and at least two shapes that satisfy both authorities.

**The one case where a prohibition genuinely does override the design's frame, and it is narrow:** §2d's injected-chrome row and the `shell_role.forbidden_chrome` rule above. Those are about what an **ancestor layout injects at runtime** — the design frame is not being departed from at all, and the fix routes to scoping the ancestor layout, never to changing what the design draws. That is why it stays an impl-side Tier-1 row and not a sendback item.

**Neither side is recorded as having won until the designer answers.** A conflict held as interim is an open loop, and §9's "Departures sent back to the designer" section says so out loud rather than letting "PR merged" read as "the design is met."

### 2f. Frame-coverage rows — every frame the designer drew must exist in the impl (brief suggestions not drawn are notes)
...
1. **Brief §7 Surface Inventory — the SUGGESTED frame set.** Read the active brief's §7 for this page (the design-handoff artifact for `{page-slug}`). Enumerate frames by **Frame name** — the contract key. Use this whenever a brief is available (brief-driven and synthesize-bundle runs).
...
| Component | State | Property | Design | Implementation | Delta |
|-----------|-------|----------|--------|----------------|-------|
| Frame: {frame_name} | — | `frame present` | brief §7 (suggested) / bundle | {drawn in bundle? built in impl?} | {✓ \| `SUGGESTED FRAME NOT DRAWN — designer's call (advisory note)` \| `FRAME MISSING in impl → Tier-1`} |

Rules:
- **A brief-suggested frame absent from the bundle is an ADVISORY NOTE, not a gap** (`shared/brief-binding-contract.md` §4, 2026-09-19 — the brief's §7 frames are suggestions; which frames to draw is the designer's call). Mark it `SUGGESTED FRAME NOT DRAWN — designer's call`, count it in `{frame_uncovered_count}` (separate from `{delta_count}`), and list it in §9 under advisory notes. Do NOT route it back as a defect and do NOT infer the drawer to "fill the gap" — `design-implement` cannot draw a drawer the designer never rendered; if the build genuinely needs one, that is a question for the designer, surfaced in §9. The exception is a frame a Part 2 truth test depends on; that case already halted at step-01 §SHARED.1b.
...
4. **Never "resolve" the debt by declaring 1:1.** Finding the token in a per-screen stylesheet is not evidence the mapping is canonical — it is the evidence it is *non*-canonical. The bundle being generated from that same per-screen CSS does not launder it: a bundle-diff cannot certify token *architecture* any more than it can certify policy conformance (§2e). **This is a measurement limit, not a ranking** — the row is a DISCLOSURE routed to `design-review`, and it never licenses overriding a design treatment (§2e "The boundary").
...
3. **Policy-declared value** — `{app_canonical_scale}.policy_declared_value` (step-02 §5a) — what `docs/design-policy.md` DECLARES canonical (§4 "Body text is 13px by default"). **The policy is the REFERENCE for a foundation-token migration**, and the app token is merely "what exists," which can diverge from it. **That is a statement about which value a routed migration targets — not a ruling that the policy outranks the design** (§2e "The boundary"). Where the design system's foundational value and the policy's declared value genuinely disagree (`kind: design-vs-app` spanning all three), the row is still routed to `apply-design-policy-change`, and the *design* half of that disagreement is a `⊘ interim(policy-conflict: …)` sendback item under step-04 §5c — never settled here by preferring one number.
...
- **An all-green grid that silently omits a whole frame (§2f).** The brief §7 promised an `order-drawer` and a `warehouse-lookup` drawer; Claude Design drew only the worklist; the component sweep cataloged the worklist's components, matched them all, and reported success — while the drawers ship *inferred* and thin (bare `€60`, code/type/status stub). A frame the bundle never drew produces zero component rows, so "all green" is "we never looked." The Frame-coverage axis is mandatory because the component sweep, like the page-shell case, is structurally blind to what was never drawn. Never infer the missing frame to close the gap. Since 2026-09-19 a brief-suggested frame the designer chose not to draw is an advisory note (`SUGGESTED FRAME NOT DRAWN — designer's call`), not a defect to route back — but it still gets a row, so the omission is visible rather than silently green.
