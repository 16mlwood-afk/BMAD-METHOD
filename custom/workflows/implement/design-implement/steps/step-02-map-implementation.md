...
Store as `{impl_page_shell} = { effective_width: "<px>" | "full-bleed", centered: bool, padding: "<value>", chain: [ {layer, max_width, centered, padding}, … ], injected_chrome: [ {layer, kind: "hero"|"banner"|"masthead"|"promo-bar"|…, summary, in_design_frame: bool} ], source: "mapped-impl" | "sibling-convention" }` — `source: "mapped-impl"` when the walk above resolved an existing page.

For a `mapped-impl` page, sibling pages are a useful cross-check: if every other feature page caps at the impl's inner width, note it — a design that wants THIS one full-width is then a deliberate divergence from a house convention, surfaced by step-03 §2d (never a silent sweep that widens every sibling).

**NET-NEW page — the impl side is EMPTY, so DERIVE the shell from the house convention (MANDATORY, not an optional cross-check).** When `{impl_page}` is a brand-new surface (nothing built yet — the step-02b "new surface, no production capabilities" case), there is no wrapper chain to walk, so the walk yields no `effective_width` — and an **empty impl value is exactly what makes the §2d page-shell row silently no-op** (design `full-bleed` vs *nothing* → no delta → green), which is how a net-new operational page ships full-bleed against a capped house convention (the `/vat-filings` miss). This is the page-shell twin of §2f's "no brief is not no contract": **no existing page is not no baseline — the house convention IS the baseline.** So you MUST read **2–3 sibling operational pages of the same `page_mode`** (the table-first worklist siblings — e.g. `/avask`, `/order-summaries`, `/invoices`) and resolve their container treatment: the `max-width` cap (or full-bleed), `margin:auto` / `mx-auto` centering, and horizontal padding. Store THAT as `{impl_page_shell}` with `source: "sibling-convention"`, and record which siblings you read. This is readable evidence the run can already reach (the same layout/page files this §1a walk reads) — an **add, not a cede**. Step-03 §2d diffs the design against this sibling baseline and treats the bundle's full-bleed wrapper as a rendering artifact, not a container decision.
...
- **Page-shell wrapper chain walked (§1a) — `{impl_page_shell}` populated with the EFFECTIVE container width after every nested layout cap, centering, and padding (OR, on a NET-NEW page with no existing impl, DERIVED from 2–3 sibling operational pages — `source: "sibling-convention"` — never left empty, so §2d has a real impl-side denominator), AND `injected_chrome` capturing any hero/banner/masthead an ancestor layout renders above `{children}` that the design frame doesn't contain**
