...
**Note — decide-one operational surfaces (pointer, not a second seam).** If `{felt_want}` describes a **per-item decision surface** — a triage queue feeding a single-item decision workspace (grading/receiving bench, claim-filing, approval queue, per-unit review/pricing/publish, mapping/triage) rather than a scan-many worklist — do NOT re-derive the cockpit doctrine here. `design-handoff` §5a owns that detection (sets `composition: operational-cockpit`) and applies the canonical **`operational-cockpit`** skill. Surface it as a one-line pointer in the route ("this reads decide-one, not scan-many — `design-handoff` will apply the `operational-cockpit` archetype per policy §6"). Stay inside the decide-one seam: a scan-many worklist is the ordinary `design-policy-canonical` §F operational table, not a cockpit.
