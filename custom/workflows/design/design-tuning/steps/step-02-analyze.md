...
- **Role catch-all (applies to all modes).** Scan for any additional element that plays one of these roles and inspect it against that role's rules **wherever they appear**, not only on the enumerated element:
  - **status indicator** — pill, badge, dot, row tint, left-border, **progress / meter / lifecycle-stage segment, chart series, coverage strip**. Apply §3 shape/dot/tint AND the full color rules (4-color cap, palette, no rainbow stage mapping, meaning→tone, same-state-same-tone) via the §2 status-color row.
