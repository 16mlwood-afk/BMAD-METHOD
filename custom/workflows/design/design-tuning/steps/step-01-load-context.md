...
- `{controls_attention_tests}` — **TC1, TC2, TA1, TA2, TF1**, present in Part 2 of every outcome-first brief (`{project-root}/_bmad/bmm/workflows/design/shared/controls-and-attention.md`, STD-CONTROLS-ATTENTION-001): a control earns its press · an instruction implies a control · emphasis follows consequence · one fact, one place · a defect is not a workload. They are ordinary truth tests and go into `{truth_tests}` with the rest; capture them separately as well so §2c can check them on a legacy or brief-less run, where the generic five apply and the report says no brief supplied this surface's own subjects. Also capture `{expected_controls}`, `{background_work}` and `{fault_categories}` from brief §4f-c — **advisory evidence**, never a requirement to draw a control.

- `{detail_surface_tests}` — **TD0/TD1/TD2-{slug}** and the brief's Part 2 **Detail surfaces** block (answer · next action · reading order · justified repeats) for every drawer or panel the brief names (`{project-root}/_bmad/bmm/workflows/design/shared/controls-and-attention.md` §2a). They are ordinary truth tests and go into `{truth_tests}`; capture the blocks separately so §2c can judge each detail surface against its own order. None on the brief → `[]`; a legacy or brief-less run still judges any drawer shown against the generic three.
