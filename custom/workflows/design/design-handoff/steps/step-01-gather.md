...
2. **Bench / desktop-only class → AUTO-FILL from policy (decided).** If the resolved class is the project's desktop-only bench class (policy §8.2 marks it DECIDED): fill the contract from policy §8.2 verbatim — `{primary_viewport_class}` = `desktop-only`; `{viewport_breakpoints}` = `desktop-only ≥1280px, landscape`; interaction = keyboard + hardware scanner (scanner-first); `{viewport_min_tap_target}` = n/a; `{viewport_device_exclusions}` = `phone, tablet — a mobile / faux-mobile UI here is a policy VIOLATION` (cite the project's clerk-web-mode hard-failure). Then run the validation gate (step 5).
...
5. **Validation gate (runs on both paths) — three classes, don't conflate them:** **HARD FAIL** (set `{viewport_present}` = false, brief NOT deliverable, revise naming the failing check): (a) `{viewport_surface_class}` unresolved; (b) **missing/partial** — a field blank on a clerk surface, or on an owner surface whose ambition is SET; (c) **policy contradiction** — a field contradicts §8, including a desktop-only bench-class surface marked mobile/tablet-supported (that fails, it never warns). **WARN-ONLY** (set `{viewport_present}` = false + `{viewport_pending_policy}` = true, brief `unverified` / `pending-policy` but STILL DELIVERABLE and the handoff CONTINUES): (d) owner ambition OPEN (step 3). A `page` run is *fully captured* only when the gate passes clean (none of a–d); it is deliverable on (d)-only, but never on a/b/c.
