...
- `{is_live_process_surface}`, `{runtime_behavior_contract}` — the §3c live-process pass (lifecycle · per-item states · transport/staleness · control verbs · progress signals); contract empty when the gate didn't fire
...
| 1, **different** `target_slug` | **HALT by default — but offer the explicit rename path** (do NOT silently fork, and do NOT auto-supersede across slugs — you cannot assume two independently-named briefs carry the same intent). A different-slug `active` brief already targets this surface (the slug-EXACT collision class, `docs/fork-gaps.md`). Surface the predecessor's path and the **two** deliberate resolutions: **(a) deliberate rename / re-issue** — when this run IS a re-issue of that same surface under a cleaner slug, re-run carrying an explicit supersede directive (`--supersede <predecessor-filename>`, or set THIS brief's `supersedes:` to that filename before generation). With that directive present and naming the same-surface predecessor §1a found, treat as `material_revision`: `{change_class}` = `"material_revision"`, `{supersedes_filename}` = that predecessor, `{predecessor_path}` = its absolute path, AND record a slug-rename note (`slug renamed {old_slug}→{new_slug}`) in the changelog + the §10 close-out. **(b) align the slugs** — give this run the predecessor's `target_slug`. The directive is what makes the rename *deliberate* (the human/agent explicitly asserts same-surface, same-intent), so 462's "don't auto-supersede across slugs" still holds; the HALT names path (a) so a mis-slugged predecessor is never a permanent trap (`docs/fork-gaps.md` §1a slug-rename). |
...
**Soft cross-route duplicate check (WARN, don't halt) — the portable residual catch.** The surface-identity match above only catches a duplicate when both briefs express the *same* `route` string. Two authors can describe ONE surface with *different* route strings (`/raw-records` vs `/ingestion-runs` for the same raw-records surface) — those resolve to different surface ids and slip the match. As a cheap backstop, compute a **slug stem** for THIS brief and every `active` brief: lower-case `target_slug`, drop a leading `owner-`/`clerk-` role prefix, take the first `-`-delimited token (`raw-records-surface` → `raw`, `owner-ingestion-runs` → `ingestion`). If another `active` brief shares this stem but resolves to a DIFFERENT surface id, **WARN** — list the suspects and ask the user to confirm they are genuinely distinct surfaces before proceeding. Do NOT halt: the stem is a heuristic and can false-positive on unrelated surfaces that happen to share a first token. This is the portable, stack-agnostic half of the surface-register idea — it reads only brief frontmatter, no route tree, so it works in every project regardless of framework. A project MAY additionally maintain a generated route+brief register (covering the built-route half) and consult it here; that register is project-local tooling, not part of this contract.
...
- [ ] **Runtime behavior captured without leaking widgets (live-process surfaces only).** When `{is_live_process_surface}`, §2c is present and renders the full `{runtime_behavior_contract}`: every lifecycle state from the implementation's own state machine (none invented, none dropped), per-item failure/partial lanes, an explicit staleness budget matching the real update transport, control verbs as outcomes with their real semantics, and available progress signals — **no item names a progress bar, spinner, log panel, or any widget** (a mechanics leak fails this check). Every §2c lifecycle state that changes what the operator sees/does has a matching `{primary}--{state}` state-variant row in §7 AND id in `frames` (the film-strip contract — a state named but never framed ships un-designed). Control verbs that invoke actions are accounted for in the mutation-derivation audit. Unresolved runtime semantics appear as Open Questions, never guessed. When `{is_live_process_surface}` is false, §2c is omitted entirely (no heading).
...
- [ ] **DO-NOT-READ list is present and complete (redesign scope only).** When `{feature_scope}` = "redesign", §8 renders the 🚫 DO-NOT-READ block naming every file that renders the target surface's CURRENT markup/structure (the view entry point, its section components, any layout-owning module) — gathered in step-01, not guessed here. The preamble's "Repo read protocol" and §8's block must agree (both present, or both in new-scope form). A redesign brief that invites repo connection without naming the view files as off-limits is the anchoring hole this check exists to close (bison-ops invoice-run-console, 2026-07-03: Claude Design read `popup.html` and shipped a re-skin — mode tabs the brief had scoped out included). For `{feature_scope}` = "new", the block is omitted entirely.
