...
### F. Analytic depth (render the shape like an analyst, not a schoolboy)

{A–E fix the band's *shape*; this fixes its *depth*. A correctly-shaped band still fails if every figure is a naked point estimate with no baseline and no read — *correct and useless*. Rendered from the `analytics-rigor` skill's captured decision (step-01 §5c-2) — do NOT re-derive. Omit this subsection only if `{rigor_read_sentence}` and `{rigor_decision_numbers}` are both empty, i.e. the surface genuinely carries no decision.}
...
- **Decision-bearing numbers — each carries uncertainty AND a base rate:** {render `{rigor_decision_numbers}` — per metric, the range / confidence / assumption it must show AND the baseline it's shown against (portfolio median / category norm / own history). A bare point estimate on any of these is a defect, not a style choice.}
...
- [ ] **Section 4b is correct.** Section 4b (Analytics Structure) is present iff `{has_analytics_band}` is `true` (band_provenance ∈ inherited | recommended-new). When present, all six subsections (A archetype & job, B reading passes, C drill behaviour, D palette & status rules, E prohibited patterns, F analytic depth) are filled with feature-specific values — no template placeholders remain. The archetype is named and matches `{analytics_archetype}` from step-01; subsection A grounds it (data dimension + user question); B's reading passes follow that archetype's form rather than a defaulted trend strip; every analytics element named in B or C has a stated drill target in C (no ornamental elements); F renders the §5c-2 rigor spec (the lead read, each decision number's uncertainty + base rate, the deciding field per chart) and surfaces — never fabricates — any data gap. When `{has_analytics_band}` is `false`, section 4b is omitted entirely.
