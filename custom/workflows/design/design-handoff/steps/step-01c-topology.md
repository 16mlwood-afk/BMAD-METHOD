...
### 5f-a. State material — §5f produces NAMES; this produces what is behind them

§5f emits state frames as names (`{primary}--{state}`). A name is enough for a conformance gate and not enough for a designer: it says a state exists, never what the surface looks like in it. **On 2026-09-25 a brief named its state frames correctly, the gate later failed the returned design by those same names, and the only picture that travelled was the populated page** — so the designer drew the populated page and dropped five preserved claims, every one of them living in a state that had never been on screen. Their own departures list named four departures and no missing state, which is what invisible looks like rather than declined. Derive the material **here**, where the frames are, or brief **§2g** renders as a heading nobody fills.

**Build `{state_material}` — a union, never one source:**

1. **Three universal rows, always present, whatever the surface does.** `empty` (nothing in it), `far-end-unreachable` (a system this surface reads did not answer), `loading` (mid-read). These are a property of **any surface that reads anything** — not of a live-process surface — so they are derived even when rules 4 and 5 emit no frames at all. That is the gap this section exists to close: an ordinary worklist gets no state-variant frame from either rule, and its empty and unreachable states would otherwise reach the designer only as a sentence in §2e about what a blank MEANS. Where one genuinely cannot arise here, **still emit the row** and say why in `what_is_true` (`cannot arise — …`); never drop it, because a dropped row and an unconsidered one are indistinguishable downstream.
2. **One row per state-bearing frame in `{spawned_surfaces}`** — every `{primary}--{state}` entry from rules 4 and 5 — deduped against (1) by state name.
3. **One row per state a `{blank_semantics}` `nobody-looked` field or a `{far_end_figures}` entry can put the surface into**, where (1) and (2) do not already cover it. §2e says what those blanks MEAN; this says what the surface LOOKS LIKE while one is true.

**Each entry: `{ state · what_is_true · how_to_see · material }`.**

- `what_is_true` — what is true in this state and not elsewhere, as an outcome. Not *"the list is empty"* but *"nothing is stuck, or nobody could look, and the two must not render the same"*.
- `how_to_see` — the route, dev flag, fixture, seeded record or query that renders it **in this repo**, read from the code and never invented. `no route — {why}` where there genuinely is none; that answer is itself worth having, because it tells the next author what a capture would cost.
- `material` — the path to a capture of that state that travels with this brief, **or** `NOT CAPTURED — {reason}`. **Never capture a state by breaking a live far end**: a fixture, a seeded record or a declared absence all beat an outage.

**`NOT CAPTURED` is a legitimate and frequently correct outcome. The requirement is that it is DECLARED, never that a capture exists.** Nothing here can make a handover contain a state nobody captured; what it can do is turn a silent omission into a named one, and that is the whole of the available mechanism. Do not let the brief claim more for it than that.
