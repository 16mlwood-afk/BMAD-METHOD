...
**Governing principle — the brief binds only through its tests — CRITICAL (2026-09-19).** The owner's words, verbatim: *"the biggest takeaway is claude design should do the heavy lifting everything else is mostly advisory"*. The brief is outcome-first in five parts — **the moment** (who opens the page, after what, to decide what) · **what must be true** (pass/fail tests, never mechanisms) · **what must dominate** (ONE thing; everything else available-on-demand) · **the data and its defects** · **open questions, unfenced** (with an invitation to sketch two options) — followed by an **advisory** appendix. The designer is bound ONLY by the Part 2 tests (T0, the five-second answer test, plus T1…Tn). Frames, layout, composition, tokens, component guidance, and the style/layout/composition rules of the project design policy are advice; truth and data rules from the policy stay binding as Part 2 tests. Full contract: `../shared/brief-binding-contract.md`. Evidence: `docs/design-brief-notes-evidence-2026-09-19.md` in the fork. Where anything below reads as a mandate on the DESIGN's form, this principle outranks it.

**Anti-Bias Principle — CRITICAL, and SCOPED TO `fresh-design` (v2, 2026-09-02):** The current UI was built by a developer, not a designer. Its layout, information grouping, visual hierarchy, and component structure are *implementation choices*, not design requirements. **In `fresh-design` mode** the brief must **never** describe what the current page looks like or how information is currently organized. Instead, give the designer the raw materials — data model, user purpose, constraints, visual direction — and let them create their own vision. The brief describes the desired aesthetic (theme, reference products, tokens), not the current structure.

**It does NOT apply in `policy-delta` or `elevation` mode, and applying it there is a defect, not caution.** Those modes run against a surface whose composition is already settled and largely working; the driver is a policy movement or a deepening opportunity, not a belief that the surface is wrong. Withholding the settled structure there does not remove bias — it forces re-derivation from blank, and **re-derivation drops things**. Measured 2026-09-02 (cash-recovery, `/listings`, policy v49 → v57): a re-derived brief lost five capabilities the predecessor already carried — `previewOfferAction`, `relistAction`, `acknowledgeManuallyCatalogedAction`, `recordWrongItemOverrideAction`, `withdrawApprovalAction` — and only an adversary review caught them. In those modes the predecessor brief's frames, carried states and capability crosswalk are an **INPUT**, and every one is retained unless explicitly retired with a stated reason.

**Anti-Bias Principle II — policy defaults are bias too — CRITICAL:** The project design policy supplies two different things, and they carry different authority. Its **truth and data rules** (money basis, no invented figures, two authorities kept distinct) bind the design as Part 2 tests. Its **visual system** (tokens, colour, type, component treatment, style hard failures) is the natural starting point and is POINTED TO, not copied — advisory to the designer since 2026-09-19 (governing principle above). The policy also attaches a **default composition to each page-mode** (operational → table-first worklist + right-side detail drawer; analytical → chart-led; detail → record-view), and that default is *not* a certification that THIS surface's job fits it. Stamping the mode's default composition into the brief unquestioned is a bias as real as inheriting the legacy layout — and harder to catch, because it feels like correctly following the system. A pull-based dispensing queue, or a per-item comparison surface, can be fully "operational" and yet be actively harmed by a table-first worklist + right-side drawer. §5a (Composition Fit Check) decides the *primary composition* from the job, and surfaces a `recommended-alt` for veto when the default doesn't fit. **Inherit the visual system; verify the composition.**

**Deliverable-Completeness Principle — every spawned surface is OFFERED, so none is lost by omission (revised 2026-09-19):** The inventory below is still derived in full and listed in §7 — but as SUGGESTED frames. Which ones the designer draws is the designer's call (governing principle above); a suggested frame not drawn is reported downstream as a note, never a failure. What follows explains why the inventory exists at all. A page does not render in isolation — it spawns secondary surfaces at runtime: the right-side **detail drawer** the operator drills into, and the **§13 expand-in-context lookup drawers** that open over it (a warehouse, a catalog record, a supplier — the foreign record read through a relation). These are part of THIS brief's deliverable, never a separate "pending" brief and never left to the implementer to improvise. The downstream pipeline is **non-interpretive by design**: `design-synthesize` / Claude Design renders only the frames the brief's §7 **Surface Inventory** enumerates, and `design-implement` pixel-matches only the frames the bundle actually contains — so a drawer the brief never listed as a required frame is never drawn, and `design-implement` then *infers* it. That inference is exactly the thin, unformalised drawer this principle exists to prevent: bare `€60` money with no GBP/VAT basis (a `docs/design-policy.md` §15 violation that ships because the drawer was never a designed surface), a lookup drawer showing only code/type/status because its §2a inline-lookups were never specified. **If you want it built well, it must be drawn.** Step-01 §5f derives `{spawned_surfaces}` (one frame per spawned surface) and §7 renders it as the Surface Inventory — a required deliverable list, **frame-name keyed** so the same name travels brief → rendered frame → `design-implement` grid row with zero inference at any hop. Escalation to a *separate* brief is reserved for a drawer that has outgrown a drawer — a deep full-page record route (§5d `needs-detail-route`); a drawer that fits a drawer is drawn in THIS brief. The corollary holds at intake: a §13 **lookup drawer targeted directly** as a handoff is **redirected, not accepted** (step-01 §2a) — it is owned by the relation and drawn as a frame in its parent's §7, so handing it off standalone would duplicate an owned frame (multiple-active-brief invariant). And the render half is enforced downstream: `design-synthesize` step-01 §7a turns every §7 Surface Inventory frame into a rendered screen (Gate 1f halts if a frame would go undrawn), so the frames this principle enumerates are actually drawn, not just listed.

**Brief Section Order:** The moment (purpose + user) → What must be true → What must dominate → Domain data and its defects → Open questions (+ design ask) → Advisory guidance (visual direction pointer, page mode, analytics, viewport, disclosure layers, style floor, suggested frames) → Implementation files. The designer learns what the page is for and what would fail it before meeting any advice.
...
- **Precedence governs what the brief SAYS, the binding contract governs what BINDS.** The policy outranks the brief on wording; `shared/brief-binding-contract.md` decides which of the policy's rules the designer is bound by (truth/data rules, as Part 2 tests) and which are advice (style/layout/composition). Classifying a style rule as advisory is not a carve-out — the rule's wording is still quoted verbatim.
- When step-03 generates the brief, sections quoted from the policy (visual personality, the style-floor list, AI sensitivity) must appear **verbatim** — no inserted exceptions, no editorializing parentheticals, no "the codebase already does X so the designer may also" softenings. Tokens, type scale, palette and component patterns are pointed to, not quoted.
...
- `{hard_failures}` - The policy's hard-failure list, verbatim. Since 2026-09-19 each item is classified `binding` (truth-class — carried into Part 2 as a test) or `advisory` (style — carried as advice) by `shared/brief-binding-contract.md` §2; a policy calling an item a "hard failure" does not make it truth-class, and an advisory match is a note, never a failed review.
...
- `{handoff_mode}` - `"fresh-design"` | `"policy-delta"` | `"refine-screen"` | `"elevation"`. **ASKED, never inferred from prompt phrasing** (see MODES below). `fresh-design` designs from raw materials with the Anti-Bias Principle in force. `policy-delta` carries a settled composition forward and applies a policy movement — anti-bias OFF. `refine-screen` consumes a `screen-review` artifact and produces a tightly-scoped refinement brief. `elevation` deepens a settled surface — anti-bias OFF; routes via `design-elevation`.
- `{policy_delta_sections}` - policy-delta only: the sections that changed between the predecessor's `policy_version_required` and the current `version`. Rendered into the brief so the revision states what drove it.
- `{carried_frames}` / `{retired_frames}` - policy-delta / elevation only: the predecessor's frames retained, and any retired **each with a stated reason**. A frame that disappears between revisions with no reason is the capability-drop failure the mode exists to prevent.
- `{split_surface_reason}` - set only by an explicit `--split-surface <reason>`; recorded in Block A as `split_from:`. Empty is the normal case.
...
- **State material** — `{state_material}`, set step-01c **§5f-a** on EVERY run and never empty: one entry per state this surface can be in (`{ state · what_is_true · how_to_see · material }`), being three unconditional rows (`empty`, `far-end-unreachable`, `loading`) plus one per `{primary}--{state}` frame plus one per state a `nobody-looked` blank or a `{far_end_figures}` entry can produce. §5f gives the state frames their NAMES; this gives the designer what is behind them, with `material` being a path to a capture **or** `NOT CAPTURED — {reason}`. Renders into brief **§2g** (unconditional). Advisory to the designer, WARN-checked at the producer; adds nothing to `{expected_required_frames}` and no §5h halt. Rationale → state-variables.md.
...
1. An explicit flag wins: `--fresh-design` · `--policy-delta` · `--refine-screen` · `--elevation`.
2. If `design-pm` or `design-router` set `{handoff_mode}` when routing → honor it.
3. Otherwise **ASK the user, presenting the four modes as the question they each answer** — not as jargon:
   - **fresh-design** — "this surface is wrong, or does not exist. Design it."
   - **policy-delta** — "policy moved. Carry the settled composition forward and apply the delta."
   - **refine-screen** — "this screen has specific named defects."
   - **elevation** — "this is settled and fine. What would make it better?"
4. **Autonomous-mode exception, and it is narrow.** Under `autonomous_mode` do not stall — but do not fall back to `fresh-design` either, which is the most destructive default (it supersedes a working brief with a blank-canvas one). Infer from EVIDENCE, in this order: an active predecessor brief whose `policy_version_required` is **behind** the current policy `version` → `policy-delta`; a `screen-review` artifact for this surface less than 24h old → `refine-screen`; no active predecessor at all → `fresh-design`; otherwise → `policy-delta`. State the inferred mode and the evidence for it in the close-out.
...
3. **Branch on result:**
   ...
   - **Artifact found AND less than 24 hours old:** Load it. Set `{review_artifact_path}` to its absolute path. Parse the YAML frontmatter into state, then parse the body's Violations → `{refine_focus}` (preserve V-IDs, severities, the **`Binding: truth | advisory`** field, and all per-violation fields — a violation from an older artifact with no `Binding:` field is classified here with the one-question test in `shared/brief-binding-contract.md` §2, ambiguous → `advisory`, and the brief says it was classified at intake), Edge States → `{required_variants}`, Peer Steals → `{peer_steals}`, Keepers → `{already_fine}`. Proceed to step-01.
...
**Refine-screen rule:** The brief produced in this mode must be BOUNDED. It addresses the artifact's top 3 violations — **ordered by binding first, then severity**: every `Binding: truth` violation comes before any `advisory` one, and **no truth violation is ever deferred** (if there are more than three, all of them are carried and the count rule yields; each becomes a Part 2 test). Advisory violations fill the remaining slots and are carried as the review's recommendation, the designer's call — never as a condition the design must meet (`shared/brief-binding-contract.md` §4) and requires variants for the artifact's edge states. It does NOT redesign the IA, does NOT introduce new components unless required to land one of those top 3, and does NOT propose a "get radical" alternative. Open creative freedom belongs in `fresh-design`. Lower-severity violations (V4+) remain in the artifact for visibility but are not in-scope for the brief unless the user explicitly asks.

**Collapse allowance (one collapse max).** Strict severity ordering can fill all three slots with mechanical token / class-swap violations (hex literal cleanup, `rounded-full` → `rounded-md`, `uppercase tracking-wide` removal, etc.) when a higher-leverage design-requiring violation sits at V4+. The brief is meant to drive design work, not enumerate find-and-replace. Therefore: **if two or more of the top 3 violations are mechanical (no design decision required — concrete class swap fully specified by the policy)**, collapse them into one combined fix labeled `Vx+Vy (combined)` and promote the next design-requiring violation into the freed slot (truth before advisory — see the severity floor below).
...
- **Severity floor.** The promoted violation must be `Binding: truth` (which is the only binding that may carry `hard failure`) — never promote an `advisory` `major` or `minor` into the top 3 just to fill the slot. If the next-highest truth violation also turns out to be mechanical, do NOT collapse a second time; emit the brief with only 2 design-requiring fixes and a combined entry, and surface to the user that the artifact's truth-violation list is mostly mechanical. A truth violation is never collapsed into an advisory one — the combined entry takes the stronger binding of the two only when both share it.
