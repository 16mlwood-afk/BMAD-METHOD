...
- `{injected_archetype}` - one of the nine archetypes, or empty. Set from `--archetype` (or an `analytics-placement-triage` handoff). When non-empty, §5c sanity-gates it (§5b-style) then honors it as the chosen archetype with provenance `injected-by-triage` and skips the `select` re-invocation — the shape was already chosen by the same skill upstream, so this avoids redundant re-derivation and threads provenance. Empty (default) → §5c selects normally.
- `{feature_scope}` - "new" (design from scratch) or "redesign" (improve existing)
- `{project_phase}` / `{is_greenfield}` - the project lifecycle phase from config (`greenfield` | `brownfield` | `mixed`; absent ⇒ `brownfield`). `{is_greenfield}` = true iff `greenfield`. Governs step-01 §1c source binding: greenfield gathers §2–§4 from PRD + architecture + UX + the design-policy-as-spec (no built code), and step-02 is skipped. The brief shape is phase-agnostic — only the INPUT SOURCES change (codifies `GREENFIELD-BRIEF-DERIVATION.md`). A greenfield brief is stamped `revision_mode: spec_derived` / `last_modified_by: human` in step-03 (`brief-revision-policy.md` §4).
...
- `{dropped_capabilities}` - Capabilities the current surface exposes that are **deliberately NOT carried into this brief**, captured in step-01 §3's mutation-derivation audit. Each is `{ capability (outcome phrasing) · backing_action · reason }` where reason ∈ `relocated` (to a named sibling surface) | `obsolete` | `out-of-scope-by-design`. This is the *log* half of the anti-silent-drop contract: `{must_support_capabilities}` records what the redesign must keep; `{dropped_capabilities}` records what it deliberately sheds and WHY — so a dropped capability is a visible, vetoable decision (surfaced at end-of-run by step-03 §5 and in the brief), never a silent omission. Empty list only when every action the current surface invokes is carried forward — never empty by omission. The blind spot it closes: a mutation on an existing record (resolve / remap / override / re-run) that is neither a primary goal nor an ingest endpoint, which the recall-based capability capture missed (the EOS batch-detail remap loss).
...
- `{spawned_surfaces}` - The secondary surfaces THIS page spawns at runtime, each a **required deliverable frame** in §7's Surface Inventory: the primary surface itself, the right-side **detail drawer** the operator drills into (present when `{page_mode}` is `operational`/`analytical` under the policy-default "table-first worklist + right-side detail drawer" composition — for `page_mode: detail` the primary surface IS the drawer), and one frame per `{linked_records_inventory}` entry (the §13 expand-in-context lookup drawers). Derived in step-01 §5f from `{page_mode}` + `{composition_provenance}` + `{linked_records_inventory}` — NOT recalled. Each entry: `{ frame_name · trigger (how it opens) · render_as (drawer-over-{parent} | full-bleed) · must_contain · figures (the §4d decision numbers it carries) · lookups (depth-1 §2a fields) }`. Rendered by step-03 into §7 as the Surface Inventory table, **frame-name keyed** so each frame is a discrete deliverable Claude Design must draw and `design-implement` must match — never inferred (Deliverable-Completeness Principle). **Depth-1:** a lookup drawer lists only its own immediate lookups; the foreign record's own §2a owns the next level (stops the recursive order→catalog→supplier graph from inlining). Empty only for a true leaf surface with no drawer and no linked records.
...
- `{analytics_archetype}` - The *shape* of the analytics band: one of `trend`, `distribution`, `composition`, `ranking`, `coverage`, `flow`, `waterfall`, `single-metric`, `correlation` (or `unclear` → ask). Empty when there is no band. **Selected in step-01 §5c by invoking the `analytics-surface-architect` skill** (the single selection brain; `shared/analytics-archetypes.md` is its taxonomy SoT). Chosen from the user's question, never the data's availability. Prevents every band defaulting to the same trend-strip-of-small-multiples.
