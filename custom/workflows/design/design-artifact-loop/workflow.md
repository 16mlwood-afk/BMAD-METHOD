...
The canonical, locked schemas for this workflow's four output artifacts live in **`templates/`** — `screen-review.md`, `design-handoff.md`, `design-response.md`, `design-tuning-state.md` (matched to `## OUTPUT ARTIFACTS` A–D above). They are intentionally simpler than the richer `screen-review --artifact` schema `design-review` emits — this workflow trades machine-parseability for cross-run consistency. Render the relevant template verbatim. The inline copies that used to live here were removed 2026-06-10 (digestibility); `templates/` is the single source of truth.
