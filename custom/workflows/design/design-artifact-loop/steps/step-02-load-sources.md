...
- `{brief_detail_surfaces}` — the Part 2 **Detail surfaces** block, one entry per drawer or panel (answer · next action · reading order · justified repeats), with its **TD0/TD1/TD2-{slug}** rows (already in `{brief_truth_tests}`; `controls-and-attention.md` §2a). A screen-review judges each opened detail surface against its own entry: the five-second answer, the reading order (provenance collapsible or visibly secondary, a qualifying caveat kept beside its figure), and one fact once. A break is `Binding: truth`. The mechanism (collapse, tab, weight, position) is advisory.
