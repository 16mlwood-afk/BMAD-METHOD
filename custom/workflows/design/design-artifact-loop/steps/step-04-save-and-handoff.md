...
Gate 5 reminder: after implementation, post-implementation acceptance requires a screenshot review (design-artifact-loop in review-only or refine-screen mode, or design-review). The next verdict must be one of: FAIL | PASS WITH NOTES | PASS. No implementation is considered accepted without this verification.
```
