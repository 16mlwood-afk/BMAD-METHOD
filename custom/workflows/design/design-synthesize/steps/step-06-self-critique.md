...
3. **TC1, TC2, TA1, TA2, TF1 — controls, attention and faults.** Standard: `{project-root}/_bmad/bmm/workflows/design/shared/controls-and-attention.md` (STD-CONTROLS-ATTENTION-001). These five are Part 2 tests and are recorded exactly like T1…Tn; they are written out here because they are on every brief and are judged against what this bundle **drew**, so the synthesizer catches them in its own output before anyone else sees it. Owner, 2026-09-20: *"drift between us thinking there's a live offer versus Amazon not receiving the offer is just a back end bug. Why is it classified as a common operational problem?"*
   - **TC1** — enumerate every action control in the rendered screens (skip navigation, filter, sort, expand, open). For each, state what the press tells the system that the system does not already have. *"Nothing — the system could run this itself"* is a **fail**; the repair is background work plus a line saying when it last ran. Cross-check the brief's §4f-c background-work list.
   - **TC2** — every instruction in the drawn prose needs a control on the same screen that performs it. An instruction with no mechanism is a **fail**; the repair is either the control, or a sentence that states the fact and stops instructing.
   - **TA1** — rank what the eye reaches first in the step-05 screenshot against what costs most if pressed or missed. A mismatch at the top is a **fail**, and so is the single most consequential control drawn as chrome.
   - **TA2** — count where each of the three loudest facts appears at rest. The same fact twice is a **fail**. The shapes this synthesizer produces most often: a group header restated in every row, two columns carrying one fact in two voices, per-row provenance that belongs to the page.
   - **TF1** — for each category the bundle counts as work, ask who must act for it to stop recurring. An engineer, a rule or a producing system means it is a fault and must be drawn as one, owned elsewhere and counted apart. A control that exists only to compensate for a fault is an independent **fail**.

   **Do not let these become style findings.** Colour, shape, placement, spacing and wording are excluded from the standard on purpose and stay in the advisory sub-checks.
