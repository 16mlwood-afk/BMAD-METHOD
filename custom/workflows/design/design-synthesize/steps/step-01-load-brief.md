...
```
Gate 1 — brief provenance: active-uniqueness invariant violated for surface "<normalised_route>[#<surface_part>]" (mode <mode>)
...
| Part 2 — TC1/TC2/TA1/TA2/TF1 | `{controls_attention_tests}` | Present on every outcome-first brief (`shared/controls-and-attention.md`, STD-CONTROLS-ATTENTION-001): a control earns its press · an instruction implies a control · emphasis follows consequence · one fact, one place · a defect is not a workload. Load them with the surface's own subjects as the brief phrases them, and let them shape step 4 BEFORE drawing — a control this bundle cannot justify should never be drawn, and a fault should never be drawn as a queue of operator work. Judged in step-06 §4t. Legacy brief: absent — record `no controls tests declared` and apply the generic five, never silently skip them |
| Part 2 — §4f-c evidence | `{expected_controls}`, `{background_work}`, `{fault_categories}` | ADVISORY evidence, not instruction. It says which controls the surface is expected to need and why each earns its press; where to put one, what it looks like and what it says are the synthesizer's call, and declining one is legitimate if said in `{advisory_notes}` |
