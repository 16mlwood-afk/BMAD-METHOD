...
**What binds the design is narrower than what outranks what (2026-09-19).** The precedence above decides which source's WORDING wins. What can FAIL a synthesized bundle is set by `{project-root}/_bmad/bmm/workflows/design/shared/brief-binding-contract.md`: only the brief's Part 2 truth tests and T0, the five-second answer test (owner, verbatim: *"the biggest takeaway is claude design should do the heavy lifting everything else is mostly advisory"*). This workflow IS the designer on the terminal path, so the heavy lifting is its job: suggested frames, composition and the policy's style floors are advice it weighs, and departures are recorded as `advisory_notes`. The "policy non-overridability" paragraph above still governs the BRIEF (a brief may not rewrite the policy); it does not make a style rule a pass/fail criterion for the design.
...
- `{mode}` — `fresh-design` | `policy-delta` | `elevation` | `refine-screen`. Inherited verbatim from the brief's mode field; refine-screen requires the brief to reference a `screen-review-*.md` artifact and to declare targeted vs unchanged regions. `policy-delta` and `elevation` follow the `fresh-design` path everywhere in this workflow — no prior-implementation baseline, no drift contract, no screen-review reference — with one difference: the predecessor BRIEF the current brief supersedes is a design input, while the current implementation still is not (`brief-revision-policy.md` §2 `mode` row). This is the **synthesis mode**, distinct from `{page_mode}` below.
- `{page_mode}` — `operational` | `analytical` | `detail`. The **page composition mode** from policy §6 / §7. Operational = table-first. Analytical = chart-first with drill-down. Detail = drawer or full-page extension of an operational list, never a re-skin. Hybrid pages default to `operational` per policy §6. Extracted from the brief's frontmatter (`page_mode:` field) or, if absent, inferred from the brief's data shape and design ask. Synthesis without a declared page mode is forbidden (Gate 1) — **except for a `surface_class: chrome` brief (app-shell nav/top-bar/shell), where `page_mode` is `n/a` by contract**: no page composition applies, skill routing is `design-policy-canonical` + `{frontend_skill}` only (the finance/analytics sisters model pages, not chrome), and the visual system + anti-AI hard failures apply verbatim. See `brief-revision-policy.md` Block B `surface_class` row.
...
- `{screens}` — Ordered list of screen names the bundle will render — one `<screen>.html` per entry. Populated in step-01 §7 from the frontmatter route(s) AND step-01 §7a from the brief's **§7 Surface Inventory** (every spawned frame — the drilled detail drawer and each §13 lookup drawer — becomes a screen, frame-name-keyed verbatim, so the non-interpretive pipeline actually draws it and `design-implement` §2f matches it by name). A `len(screens) == 1` run is a true single-frame leaf only. Recorded in manifest under `screens:`.
- `{consumed_frames}` — §7 frames whose "Must contain" is `= the {X} drawer in {other-brief}.md — consume, do not redesign`: owned by a sibling brief, recorded (name + owning brief) and NOT re-synthesized here, so the step-01 §7a coverage gate (Gate 1f) counts them as accounted-for rather than undrawn. Empty when this brief owns every frame it lists.
...
- **Page mode declared up front.** Step 1 must resolve `{page_mode}` from the brief — `operational` | `analytical` | `detail` per policy §6 / §7. Per policy §6, hybrid pages default to `operational`. The page mode constrains composition (table-first vs chart-first vs detail extension), skill routing (operational → `operational-finance-ui`; analytical → `operational-analytics-band`), and what the lift test considers acceptable. Synthesis without an explicit `{page_mode}` is forbidden (Gate 1). **Exception — `surface_class: chrome`:** app-shell briefs carry `page_mode: n/a` by contract; no page composition or page-mode lift criteria apply (the design system + anti-AI hard failures still do), and skill routing is the always-invoke pair only. Do NOT infer a page_mode for chrome.
- **No design inference from existing screens.** The current implementation may pre-date the policy or be mid-migration; it is not a design source (per `design-policy-canonical` skill: "Do not infer design from existing screens"). In `fresh-design` mode, only the brief, the policy, and the project token inventory authorize design choices. In `policy-delta` / `elevation` mode the same three plus the predecessor brief the current brief supersedes — still never the current implementation. In `refine-screen` mode the prior implementation is a **drift baseline** — a reference for what NOT to alter outside `targeted_changes` — not a design source.
...
- **Synthesis honesty — claims require evidence.** Three honesty rules apply to the manifest's verdict fields:
  ...
  - **`skills_invoked` records actual skill loads, not theoretically applicable skills.** An entry in `manifest.skills_invoked` requires that the skill's content was genuinely loaded into context during this run — normally via the Skill tool; in a worktree, alternatively via a direct Read of the MAIN checkout's `.claude/skills/{name}/SKILL.md` when the skill is synced-but-untracked and therefore invisible to the worktree (recorded with `loader: main_checkout_read` — see step-04). Operating "in the spirit of" a skill — applying the workflow's summary of its rules without loading the skill itself — is NOT invocation. Skills mandated by the routing matrix but not actually loaded are recorded in `{skills_unloaded}` with a reason; the bundle's `compliance_state` becomes `under_grounded`.
...
| `n/a` (`surface_class: chrome`) | — none beyond always-invoke (`design-policy-canonical` + `{frontend_skill}`) — the finance/analytics sisters model PAGES and are **not applicable** to app-shell chrome | — |

**Refine-screen mode** uses the same page-mode-driven matrix above. Synthesis mode (`fresh-design` / `policy-delta` / `elevation` vs `refine-screen`) does not change which skills are authoritative — page mode does.
...
The manifest is split-authority: authoritative for the synthesis receipt + interaction semantics + region declarations + flow invariants; **never** authoritative for visual properties (those live only in `bundle/<screen>.html` + `bundle/tokens.css`, read by `design-implement`).

The full field-by-field schema is **`../manifest-schema.md`** (sibling to `steps/`) — extracted from this file 2026-06-10, verbatim. step-07-emit-manifest renders it; all six `violations.*` arrays, `visual_quality_axes`, and `macro_hierarchy` are MANDATORY (emitted even when empty — omission is a workflow bug).
...
- `manifest.skills_invoked` includes the skills required for `{page_mode}` per the routing matrix (always: `design-policy-canonical` and the resolved project frontend skill; mandatory by page_mode: `operational-finance-ui` for operational/detail, `operational-analytics-band` for analytical; `page_mode: n/a` / `surface_class: chrome`: the always-invoke pair only). A missing frontend skill is a Gate 5a halt — not a success-criteria warning.
