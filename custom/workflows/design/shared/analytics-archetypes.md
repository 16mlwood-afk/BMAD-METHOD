...
>
> **Shape is not depth — `analytics-rigor` governs the second axis.** This taxonomy and its selector answer *what shape*. They do **not** answer whether that shape is rendered with an expert analyst's depth: a correctly-chosen `coverage` strip can still ship every figure as a naked point estimate, with no base rate, no magnitude on the trend, and no one-line read — *correct and useless*. That axis belongs to the **`analytics-rigor` skill**, run after the archetype is chosen (`design-handoff` §5c-2) to specify the lead read sentence, the uncertainty each decision number carries, the base rate it's shown against, and the deciding field (vs the handy proxy). Archetype = shape; rigor = depth; they compose, neither replaces the other. The cross-cutting rules below are shape rules; rigor is enforced separately (`C-RIGOR-01` at review).
>
