...
brief_shape: outcome-first               # outcome-first (2026-09-19+) | absent ⇒ legacy. Either way consumers apply shared/brief-binding-contract.md.
page_answer: "the prep's records are missing 14 of your purchases"   # T0 — the line a reader must state within five seconds (outcome-first only)
dominant: "the purchases the prep is missing"                         # the ONE thing that leads (outcome-first only)
truth_tests: [T0, T1, T2]                # the Part 2 binding test ids — the ONLY things that can fail a design (outcome-first only; never empty, T0 first)
mode: fresh-design                       # fresh-design | policy-delta | elevation | refine-screen
...
composition: worklist                    # machine-readable composition key the bundle→implement gate diffs against (design-implement §SHARED.1b check 3). Default = the page-mode default (operational→worklist | analytical→chart-led | detail→record-view); a recommended-alt sets the named job-fit key (scanner-terminal | single-item-stream | source-co-present | source-mirror | operational-cockpit | …). A non-default key means the gate verifies the bundle expresses the JOB LOOP, not a hero card. (source-mirror is the faithful-source-mirror archetype — render source rows verbatim at fidelity; §13 expand-in-context is suppressed, canonical identifiers are scan/trace anchors not lookup-drawer targets. It is always composition_provenance: recommended-alt — a mirror is NOT the operational worklist default.) (operational-cockpit is the decision-first triage+workspace archetype — a triage queue feeding a full-width single-item decision workspace per project policy §6 "Operational cockpit". The `operational-cockpit` skill is the **canonical doctrine** — M1–M6 mandatory floor (classify-first · queue+workspace co-present · per-item momentum · keyboard-first · consequence-visibility before an irreversible commit · no working blind) + H1–H5 heuristics; consumers defer to it rather than restating a checklist. Always composition_provenance: recommended-alt; unlike source-mirror it does NOT suppress §13 — it still spawns detail + lookup frames.)
...
frames: [my-feature]                     # SUGGESTED frames (advisory — brief-binding-contract.md §4): the §7 Surface Inventory ids (never empty — ≥ the primary frame). The conformance gate reports an undrawn suggestion as a NOTE, never a halt.
...
|---|---|---|---|
| `brief_shape` | `outcome-first`, or absent (⇒ legacy) | every brief from 2026-09-19 | Which body shape the brief uses. `outcome-first` = the five parts (moment · what must be true · what must dominate · data and defects · open questions) followed by an advisory appendix (`shared/brief-binding-contract.md`). Absent = a legacy brief; consumers read its `MUST PRESERVE` list as its truth tests and treat its frames/§4/§5/§7 as advisory. Never a halt on absence. |
| `page_answer` | one line | iff `brief_shape: outcome-first` | The page's answer — what a reader must state within five seconds of it loading (T0). Consumers run the five-second test against it. |
| `dominant` | one short phrase | iff `brief_shape: outcome-first` | The ONE thing that must lead the page. Everything else is available-on-demand; demoting it is legal. |
| `truth_tests` | list of ids, T0 first | iff `brief_shape: outcome-first` | The Part 2 binding tests. The ONLY items that can fail a design (`design-review-pr` C-TRUTH-01 / C-ANSWER-01, `design-implement` §SHARED.1b, `design-synthesize` step-06). Never empty. |
| `mode` | `fresh-design`, `policy-delta`, `elevation`, `refine-screen` | every brief | Which handoff mode produced this brief (`design-handoff` `{handoff_mode}`, written verbatim — never `fresh-design` as a stand-in). **`fresh-design`** — open creative scope from raw materials; the current UI is not a design source and any predecessor is superseded, not carried (Anti-Bias Principle ON). **`refine-screen`** — bounded to a `screen-review` diagnostic; the only mode that carries the refine-screen fields. **`policy-delta`** — a material revision of an ACTIVE predecessor brief driven by a design-policy version movement (the predecessor's `policy_version_required` is behind the current policy `version`). **`elevation`** — a material revision driven by a deepening opportunity on a settled surface (the `design-elevation` lane). `policy-delta` and `elevation` are the two **settled-composition** modes and share one contract: (i) the predecessor brief's frames, carried states and capability crosswalk are INPUT — every one is retained unless explicitly retired with a stated reason (Anti-Bias Principle OFF; `design-handoff/workflow.md` Anti-Bias Principle v2); (ii) they never carry the refine-screen fields — `screen_review_ref`, `targeted_changes`, `unchanged_regions`, `deferred_violations` MUST be absent; (iii) for pipeline mechanics — `design-synthesize` prior-implementation lookup and drift baseline, `design-artifact-loop` mode lock — they behave exactly as `fresh-design` (no drift baseline, no screen-review reference), with ONE difference: the current implementation is still not a design source, but the predecessor BRIEF is; (iv) being material revisions by definition they carry `change_class: material_revision` with `supersedes` naming the predecessor (invariant 5) — a settled-composition brief with no predecessor is a mislabelled `fresh-design`. For active-uniqueness the three non-refine modes are ONE bucket (invariant 6). |
...
| `surface_class` | `page` \| `chrome` | every brief (absent ⇒ `page`) | Whether the target is a content PAGE (the page_mode triad applies) or app-shell CHROME — global navigation, sidebar, top-bar, mobile nav drawer, the shell frame itself. When `chrome`: `page_mode` is `n/a` (not the triad), and the page-machinery fields (`composition`, `composition_provenance`, `band_provenance`, `analytics_archetype`) are ABSENT BY DESIGN — consumers MUST NOT enforce page-mode composition, analytics-band, data-walk, or §13 lookup expectations against a chrome brief (project design policies already scope their page-layout rules to pages, e.g. "app-shell navigation is out of scope" — this field is the workflow-side mirror of that scoping). `frames` remains REQUIRED (the chrome variants + states, e.g. `nav-desktop`, `nav-mobile-drawer`, `nav-desktop--collapsed`), and the visual system + anti-AI hard failures apply verbatim. For chrome, `route` is the shell's scope anchor (normally `/` — chrome spans routes) and `surface_part` names the chrome piece (e.g. `primary-nav`, `top-bar`) so the surface identity stays unique. |
| `frames` | list of frame ids (the §7 Surface Inventory rows) | every brief | Machine-readable mirror of the §7 SUGGESTED frame names — advisory to the designer (`brief-binding-contract.md` §4). The bundle→brief gate (check 1) reports a suggested frame the bundle did not draw as a NOTE; never empty (≥ the primary frame). Identical ids to the §7 table — kept in sync. |
| `shell_role` | `{required_shell, required_chrome, forbidden_chrome}` object | iff the app has a role/shell distinction (clerk vs owner) | The page-shell & role contract. For the DESIGN it is advisory; its data-protecting half is also written as a Part 2 truth test, which is what binds. `design-implement` §2d still flags an ancestor layout injecting `forbidden_chrome` over the BUILT surface as Tier-1 — that is implementation fidelity, not a design judgement. Omitted for single-role apps. |
...
1a. **Field completeness — Block B.** `mode`, `page_mode`, `route`, and `frames` are present in every brief (`frames` is never empty — ≥ the primary frame). `composition_provenance`, `composition`, and `band_provenance` are present in every **`surface_class: page`** brief; on a `surface_class: chrome` brief they are ABSENT BY DESIGN (with `page_mode: n/a`) and their absence is NOT a completeness failure — see the `surface_class` field row. `shell_role` is present iff the app has a role/shell distinction (clerk vs owner), omitted otherwise. `analytics_archetype` is present iff `band_provenance` ∈ {`inherited`, `recommended-new`} and absent otherwise. When `mode: refine-screen`, the additional fields `screen_review_ref`, `targeted_changes`, `unchanged_regions`, and `deferred_violations` are also present (the latter may be an empty list but the key must exist); in every other mode (`fresh-design`, `policy-delta`, `elevation`) those four fields MUST be absent — not empty, absent. `collapse_note` is conditional — required iff a collapse occurred, absent otherwise. **Backward compatibility:** a brief authored before the analytics-archetype contract (no `band_provenance` key) is interpreted as `band_provenance: none`, and a brief authored before the composition-fit contract (no `composition_provenance` key) is interpreted as `composition_provenance: policy-default` — consumers MUST treat each absent field as its default rather than halting, the same way an absent `policy_version_required` defaults to `0`. **Likewise, a brief authored before the structural-contract fields (no `composition` / `frames` / `shell_role` / `surface_part` keys) is interpreted as `composition`: the page-mode default, `frames`: UNVERIFIED (the `design-implement` bundle→implement gate discloses, never halts), `shell_role`: absent, `surface_part`: `""` (the whole-route primary surface), `surface_class`: `page` — consumers degrade gracefully and NEVER halt on their absence** (the gate only bites on a present contract; a missing one is disclosed, per the SP-API lesson). New briefs from `design-handoff` always emit `composition` + `frames` + `surface_part` (and `shell_role` when the app is multi-role). **From 2026-09-19 they also emit `brief_shape: outcome-first` with `page_answer`, `dominant` and a non-empty `truth_tests`**; a brief with no `brief_shape` is legacy — consumers read its `MUST PRESERVE` list as its truth tests, treat `frames`/`composition` as advisory exactly as for an outcome-first brief, and NEVER halt on the absence of the outcome-first fields. An outcome-first brief missing `page_answer`, `dominant` or `truth_tests` is malformed (Check 1c).
...
6. **Active uniqueness (per surface).** For a given **surface identity** — `normalise(route)` + `surface_part` (absent ⇒ `""`), evaluated within one **mode bucket** — at most one brief in `{implementation_artifacts}` may have `brief_status: active` at any time. There are exactly two buckets: **{`fresh-design`, `policy-delta`, `elevation`}** is ONE bucket — they are all *the primary brief* for a surface, so a `policy-delta` (or `elevation`) brief supersedes the `fresh-design` / earlier settled-composition brief it revises rather than coexisting with it — and **{`refine-screen`}** is its own. Uniqueness is keyed on the **surface, NOT on `target_slug`**: two differently-named slugs that target the same surface are a violation (the slug-EXACT collision class — see `docs/fork-gaps.md`). When a new active brief is written, the predecessor's `brief_status` flips to `superseded` and its `superseded_by` is set in the same edit. (`normalise(route)` = `route` lower-cased, trailing `/` stripped, dynamic segments left verbatim. A refine-screen brief and the primary brief on one route are deliberately distinct surfaces — the bucket scoping keeps them apart.)
...
2. **Decide `change_class`** by checking `{implementation_artifacts}` for a prior **active** brief on the **same surface** — same surface identity (`normalise(route)` + `surface_part`, within `mode`), **NOT** same `target_slug` (see invariant 6 and `step-03-generate-brief.md` §1a):
   ...
   - Exactly one, **different** `target_slug` → halt **by default**: a different-slug active brief already targets this surface (the slug-EXACT collision class — `docs/fork-gaps.md`). Never auto-supersede across slugs (you cannot assume two independently-named briefs share intent). Two deliberate resolutions: **(a)** carry an explicit supersede directive (`--supersede <predecessor>`, or pre-set this brief's `supersedes:`) asserting same-surface re-issue → then `change_class: material_revision`, `supersedes: <predecessor>`, plus a `slug renamed {old}→{new}` note — the in-situ rename path, so a mis-slugged predecessor is never a permanent trap (`docs/fork-gaps.md` §1a slug-rename); or **(b)** align the slugs. The directive (not automation) is what keeps it deliberate.
...
- When `mode` ∈ {`fresh-design`, `policy-delta`, `elevation`}: those four refine-screen fields MUST be absent (§2 invariant 1a). A `policy-delta` / `elevation` brief additionally carries `change_class: material_revision` + `supersedes` (§2 `mode` row, invariant 5) — checked in Check 2, not here.
...
- `mode ∈ {fresh-design, policy-delta, elevation, refine-screen}` — widened 2026-09-02 (see §7 note); a consumer copy that still lists two values is stale, not authoritative
- `surface_class ∈ {page, chrome}` (absent ⇒ `page`)
- `page_mode ∈ {operational, analytical, detail}` — **when `surface_class: chrome`, the allowed value is exactly `n/a`** (the triad names kinds of page work; chrome is not a page). A chrome brief carrying a triad value, or a page brief carrying `n/a`, is out-of-enum → halt.
...
### Check 1c — outcome-first completeness (only when `brief_shape: outcome-first`)
An outcome-first brief must carry `page_answer` (non-empty), `dominant` (non-empty, one thing), and `truth_tests` (a non-empty list whose first id is `T0`), and its body must carry the headings `Part 1`, `Part 2` and `Part 5`. These are the parts that bind or orient the designer; a brief missing them hands the designer a page with nothing to rank against. Halt diagnostic: `Outcome-first brief missing: <fields/parts>. Re-run design-handoff (step-01 §3i outcome pass). See shared/brief-binding-contract.md.` **Skipped entirely for a legacy brief** (no `brief_shape`) — absence is never a halt; the consumer reads the legacy `MUST PRESERVE` list as the truth tests and notes `legacy brief shape` in its output.

`brief_shape` (when present) is closed-enum: `{outcome-first}`.
...
Compute THIS brief's surface identity — `normalise(route)` + `surface_part` (absent ⇒ `""`), within its mode BUCKET (§2 invariant 6: `fresh-design` / `policy-delta` / `elevation` are one bucket, `refine-screen` is the other — a `policy-delta` brief and the `fresh-design` brief it supersedes therefore MATCH). Scan ALL `design-brief-*.md` in `{implementation_artifacts}`, parse each one's frontmatter, and list those with `brief_status: active` whose surface identity matches. If more than one is found (including this brief), halt:
...
**Mode enum widened (2026-09-02).** `mode` grew from {`fresh-design`, `refine-screen`} to {`fresh-design`, `policy-delta`, `elevation`, `refine-screen`} (§2 `mode` row). Backward compatibility is one-directional and cheap: a brief that predates the widening carries one of the two original values, both still valid, so **no consumer halts on an older brief**. The drift this closes runs the other way — a `policy-delta` or `elevation` brief read by a consumer copy that still lists the two-value enum fails Check 1b as "out-of-enum" when the brief is correct and the consumer is stale. Where that happens the fix is the consumer copy (re-sync), never re-stamping the brief `fresh-design` to appease it.
