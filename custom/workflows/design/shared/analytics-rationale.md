...
## 3c. Which decision? Decision-analysis specification (capital-commitment surfaces only)

Present **only** when `{is_capital_decision}` is `true` — the surface commits a scarce resource under uncertainty with a downside (a buy / reorder / sizing). §3b made the figures honest (senior-analyst); this models and sizes the *decision* (executive). Captured in step-01 §5c-3 by the `decision-analysis` skill; the enforced contract is the brief §4e, which `C-DECISION-01` reads (this is the reviewer-facing reasoning record). Omit the whole section for non-commitment surfaces.

- **The bet:** {decision_frame — action · capital at risk · horizon · payoff (win / lose)}
- **Modelled outcome:** {decision_outcome — method (monte-carlo / scenario / closed-form / single-scenario) + P(success) · EV · P10 · P90 + model assumptions; or an honest single-scenario read + the VOI gap when un-modellable}
- **Sizing:** {decision_sizing — quantity · basis (loss-distribution / capital-cap / fractional-Kelly) · the downside it survives}
- **Breakeven driver:** {decision_sensitivity — the swing input and the threshold that flips the call}
- **Outside view / regime / asymmetry:** {decision_context — the computed reference class, the decision-relevant window, and the payoff asymmetry that tilts the recommendation}
- **Value-of-information & gaps (surfaced, NEVER fabricated):** {decision_gaps — the missing input that would most move the decision; a fake outcome distribution never stands in for an un-modellable one}
...
- [ ] §3c is present iff `{is_capital_decision}` is `true`; when present it records the decision spec — framed bet, modelled outcome (or honest single-scenario + VOI gap), sizing tied to the loss tail, the breakeven driver, and the outside-view/regime/asymmetry context. No fabricated outcome distribution stands in for an un-modellable decision. Omitted entirely for non-commitment surfaces.
