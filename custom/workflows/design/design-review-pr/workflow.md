...
- **`C-FIXTURE-01`** — fires when the diff touches a **fixture-backed production route**: a route that renders a fixture/mock data module (a `DATA_STATE = "fixture"` self-declaration, or a heuristic `*data*`/`*fixture*`/`*mock*` record-array module) with **no live read path** and **no visible "not live data" disclosure**. Closes the gap where a route ships convincing fabricated data behind auth that looks live. **Source-detected, not brief-derived** (a route's data source is a code fact — step-02 §7), and **graceful across projects**: it prefers the project's own deterministic gate (`scripts/check-fixture-disclosure` / the npm script) and trusts its full-graph verdict; the hard P1 fires ONLY where the project declares the disclosure contract (the script OR a `docs/design-policy.md` fixture-disclosure assertion) — a project that has not adopted the policy gets a human-judgment prompt, never a P1 (firing one there is the indiscriminate-gate failure). The **irreducibly-probabilistic** half is always a human prompt: does the fixture contain REALISTIC customer-like content/PII that could be mistaken for a real person's data (fabricated PII must be obviously synthetic) — FAIL unless a logged waiver. Evaluated in step-04 §1b-5.
