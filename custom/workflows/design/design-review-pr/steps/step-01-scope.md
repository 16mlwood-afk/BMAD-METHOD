...
- **§2d absent, or archetype unclassified, on a finance-shaped or movement-shaped route:** if the route handles ledger/transaction/inventory-movement data yet the brief has **no §2d**, or §2d carries prose instead of one of the three enumerated values (`not-a-ledger` · `register` · `running-balance`), note it in `{findings.coverage_notes}` ("route X is finance/movement-shaped but the brief does not classify its ledger archetype — the design-handoff §3g pass may not have run"). If §2d says `register` or `running-balance` **and** the project's design-policy chain defines a ledger archetype, also check the brief carries that policy's rules and — for `running-balance` — states the oldest-first sort. **`not-a-ledger` is a PASS and is never noted.** **Honest ceiling, state it in the finding:** this lane reads TEXT. A green result means *the brief said the right thing* — it can never confirm the rendered comp aligns its money column, distinguishes a true zero from a not-applicable, or carries corrections as linked entries. A comp is not a tool call and no check here can block one. Otherwise (non-finance, non-movement route) do not add an entry; not a failure, not noted (the norm).
