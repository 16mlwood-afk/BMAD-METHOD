...
- **Judging the SHAPE of a handheld-first artifact — the review-board check (`shared/operator-artifact-contract.md` B1–B7, Layer B).** Correct viewport labeling does not make an artifact an operator surface. On any artifact for a handheld-first / mobile-primary class, run five checks and **FAIL** on any of them, naming the rule id and the artifact location:
  ...
  - **C5 no dashboard opener** — the canonical render (or the live route) opens with a **hero/billboard/chip-grid band above a table-first worklist** instead of a compressed operational stack (B7). **REJECT even when every required element is present, every field is declared, and C1–C4 all pass.** Judgment-lane, MUST cite at least one concrete tell: **(1)** hero/banner row — a full-width opener block that announces rather than acts (large empty region, centred composition, its own background/border/elevation); **(2)** billboard CTA — the primary action given its own dedicated row/band rather than sitting inline in the worklist header; **(3)** chip wall — secondary counts/statuses/filters as a wrapping grid of pills or metric tiles at competing weight; **(4)** summary-card feel — a card boundary separating the header from the list so it reads as a different component rather than the top of it; **(5)** **no data above the fold** — zero worklist rows visible at rest. Write the finding as *"DASHBOARD OPENER — {tell}, {location}. B7.{n}. Compress into the worklist header; do not delete the {count/action/status}."* **The correction is compression, never deletion.**
    - **C5 has a DOM-lane sub-check — run it, don't eyeball it.** This lane already navigates the live route at the canonical viewport. At **375×812** (or the class's canonical size), count the worklist rows whose bounding box intersects the viewport at scroll-0. **Zero rows ⇒ FAIL C5 on tell 5**, objectively, with no judgment required — report the measured row count and the header block's height as a fraction of viewport height (>~⅓ corroborates). Tells 1–4 stay judgment-lane. This is the only part of the composition contract that is machine-measurable, so **measure it rather than arguing it**.
    - **C5 binds by CONTENT SHAPE, not viewport class** — any DECIDED class whose primary content is a list/table/queue/worklist, desktop included, measured at that class's own canonical viewport. It does **not** fire on a single-record cockpit with no list, or on an OPEN owner ambition.
  These are shape defects, not formatting nits: a review-board artifact passes every existing viewport, token, and component check by construction, and the implementation that follows it inherits the wrong premise. **C5 is the sharpest case of that** — a dashboard opener passes C1–C4 *and* gate (f) *and* every §4g field, because presence-checking cannot see composition. Run C5 as its own pass after C1–C4 pass, not as a tiebreak when something already looks wrong.
