...
**Goal:** Turn `{coverage_matrix}` + `{verdicts}` into an actionable report. Every contract field gets an explicit disposition — covered, gapped, and benign alike — and every gap is routed to its fix lane. Nothing is silently dropped. This workflow ends at the routing boundary; it does not apply fixes.
...
- **P1 (critical):** a whole-column-empty (`0/N`) field whose verdict is **present-but-dropped** — silent total loss of a real field; or **no declared record contract** (`{export_schema} = NONE`); or a systemic **fetch-failure** producing stub rows in the record set.
...
A genuinely-absent field is benign *only because the live page confirmed it* — that confirmation is the disposition's justification, not an assumption.
...
For **present-but-dropped**, prefer `maintenance-triage` when several fields dropped together (often one root cause — a locale, a page-layout change) and `quick-spec` when it's a single field. When in doubt, route to the producer fix over guessing it benign.

## EMIT THE DISPOSITION TABLE

Every contract field becomes one row — **including fully-covered and benign ones** (disposition `accepted`, with the reason). This is the silent-partial-implementation guard: a reader can tell, without asking, that nothing was quietly skipped.
...
- **Autonomous mode:** do not halt. Present the report and, for each non-benign finding, state the exact next workflow to run (copy-paste-ready), e.g. *"Run `/bmad:bmm:workflows:quick-spec` framed on the `Order Date` extractor — teach it the es-ES `Pedido realizado` label."* Do not invoke them — routing is the boundary.
- **Interactive mode:** present the report, then offer to kick off the top-priority route. Let the user choose.

In both cases the routed lane owns the actual change (in its own worktree, under its own rules). This workflow has delivered its value — the coverage matrix, the per-field verdict, and the route — and stops.
...
- Dropping fully-covered or benign fields from the table (reader can't tell what was skipped)
