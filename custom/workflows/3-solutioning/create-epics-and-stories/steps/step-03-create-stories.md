...
- Get user confirmation to proceed to next epic — **AUTONOMOUS MODE:** if `autonomous_mode` is `true`, skip this confirmation and continue to the next epic without pausing
...
- **AUTONOMOUS MODE:** If `autonomous_mode` is `true` in config, do NOT halt — skip this menu, treat it as `[C] Continue`, and proceed immediately (simulate an informed expert decision per the project's `autonomous_rules`; never pause to ask). The rules below apply to interactive (non-autonomous) runs only:
