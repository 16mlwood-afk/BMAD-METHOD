...
metadata:
  ...
  version: '1.2'
  last_verified_against_fork_commit: '0dd2e1ca'
---
...
- Mason-BMAD, the fork at `~/bmad-method-v6/`, `brief-revision-policy`, brief provenance, the grounding gate, `sync-bmad-workflows`, `project_phase` (the fork's lifecycle config), `maintenance-triage` (fork-only workflow), `autonomous_mode` scoping (decision vs intent autonomy)
...
```
**Root cause class:** [one of: ungrounded-input | provenance-violation | worktree-sync-drift | greenfield-assumption-in-brownfield | upstream-fork-mismatch]
...
If anything updates, bump `last_verified_against_fork_commit` in SKILL.md frontmatter to the fork HEAD after the STATUS.md update. The version (`1.2`) only bumps when the skill's public shape changes (new mode, new section, breaking change to the diagnostic template, etc.); the verification commit bumps on every sync-with-fork pass even if the content didn't change.
