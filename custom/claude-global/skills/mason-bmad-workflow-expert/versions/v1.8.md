...
**Response style — Mason-readable (DEFAULT for substantive design / governance / explain answers).** Lead with a phone-at-a-red-light **Plain answer** block (3–6 lines): a 1–2 sentence **bottom line**; then **what changed / what's true now** in plain speech; then **what I recommend now** as ~3 bullets, each an action or a decision the owner must make. THEN a short **Context & why** (1–2 paragraphs). THEN **Details / spec** (charters, tables, lifecycle, self-review) LAST — the deep content must never be required to understand the decision. Prefer concrete examples from current projects over abstract theory; scale down for trivial replies. Full rule: the global `mason-readable-design-governance-format` memory (a concrete template under `answer-shape-and-autonomy`). **Exception:** Mode 3 (Diagnose) keeps its own REQUIRED diagnostic-format template instead of this shape.
