...
- **Item C — memory probe:** SHIPPED — the SessionStart probe now also warns when a live project has no memory discipline (no project `MEMORY.md` and no CLAUDE.md memory section); `pr_has_memory` added to the shared lib; deploy and memory warn independently. Tested.
- **git-push-auto-deploy detection:** SHIPPED — the gate now also matches `git push … main|master` (space-anchored so `feature/main` is not a false match); explicit verbs (`railway up`/`redeploy`/`bmad-deploy.sh`) plus this.
- **Item E — fresh-machine install:** SHIPPED — skill + hooks backed up to `custom/claude-global/{skills,hooks}`, `install-global-assets.sh` extended to restore the hooks and idempotently register the three settings entries (gate stays DRY-RUN). Verified idempotent.
- **Remaining:** **Item D (drift / `contract_version`)** — the one genuinely-bigger piece: needs `sync-bmad-workflows.sh` to stamp each project's conformed contract version into its config so the probe has something to compare (the charter already carries `contract_version: 1`); and **the enforce flip** — a deliberate one-line switch (`echo enforce > ~/.claude/prod-readiness-gate.mode`) after the dry-run log proves clean over real deploys.
