...
- `sprint_apply_marker` records what step 6 did with `.sprint-apply-pending.json`. A proposal whose `files_to_change` has **no board-reachable execution artifact** (design-lane / planning-artifact — e.g. correct-course used only as the scope-register front door) is `NOT_DROPPED — no tracker files`: it never touches the single-slot marker. **Board-reachable means `sprint-status.yaml` or a story file at `ready-for-dev` — `epics.md` is a planning artifact and does NOT count**, so an `epics.md`-only proposal lands in this branch and is therefore legal ONLY when every row is `R5-parked` (step 6 HALTs otherwise; see FG-2026-07-31-11). If the slot is already held by a different `proposal_id`, step 6 is `BLOCKED` rather than clobbering it. Only a proposal that edits sprint-execution artifacts into a free/own slot is `dropped`.
...
<action>**NAME THE TRACKER DELTA — mandatory on every row whose `route` is NOT `R5-parked`.** State the concrete entry this scope adds to the plan and ADD that file to `files_to_change`. Without it the proposal carries no sprint-execution artifact, so Step 6 writes no pending marker, so there is no `APPROVE:` token and no executor path: the item exists only as a register row and moves only if a human remembers it. That is the observed failure this step exists to close — agreed scope sitting registered-and-inert while the board never learns it exists. A row with a `next_artifact` but no tracker delta is REGISTERED, not actionable (STD-SCOPEREG-001 §4).</action>

<action>**THE DELTA MUST BE BOARD-REACHABLE — an `epics.md` line ALONE is NOT one.** It qualifies only if it lands in **`sprint-status.yaml`** (a story key, an `epic-N` key, or the R4 milestone block) **or a story file at `ready-for-dev`**. An `epics.md` story line is a *description*: STD-SCOPEREG-001 §4 classifies it as **DESCRIBED**, which is actionable for **nothing** — no consumer can start from it. Writing `epics.md` **as well** is legitimate and often right; it can never be the delta that satisfies this step. State the delta as the literal key you will add (e.g. `sprint-status.yaml → epic-11 + 11-1..11-6`), not as a file name.</action>

> **This rule is narrower than it was because the looser wording shipped a four-day silent failure** — `docs/fork-gaps.md` **FG-2026-07-31-11**. A proposal listed `epics.md` and **not** `sprint-status.yaml` in `files_to_change`. Everything it named was applied *correctly*: a PRD requirement amended, an architecture decision ratified, the scope row routed `R1-capability` with a named `next_artifact`, and a complete epic with six stories written into `epics.md`. The epic never reached the board, so **zero code was written and the owner raised the same missing feature three times.** Every artifact a cold session reads reported health.
>
> **Do not expect a downstream check to catch this.** The board-reading tooling derives its epic population *from the board* (`build_epics()` returns only epics that already have story keys), so an epic with no rows is not reported as missing — it is **unrepresentable**. The stage banner, gap analysis and "what should I work on" all inherit that blind spot. **This step is the only place the failure can be caught.**
...
<check if="`files_to_change` contains NO BOARD-REACHABLE execution artifact (no `sprint-status.yaml`, no story file) — an `epics.md` entry ALONE does NOT satisfy this">
  ...
  **Otherwise this is a DEFECT in Step 4.5, not a legal exit.** A proposal carrying a non-parked row reached here without naming its tracker delta, which means the scope it just agreed has no executor path. HALT and surface: "Row(s) `<ids>` are routed `<route>` but this proposal names no BOARD-REACHABLE tracker delta. Per Step 4.5, every non-`R5-parked` row must name the `sprint-status.yaml` key (or story file at `ready-for-dev`) it adds and list that file in `files_to_change`. Add it, or re-route the row to `R5-parked` with a complete `activation`." Do NOT record `NOT_DROPPED — no tracker files` to get past this: that string was the silent exit that let agreed scope become inert, and it is no longer a legal disposition for a routed row.

  **`epics.md` ALONE is this same HALT, not a pass — and it is the case that actually fired.** If `files_to_change` names `epics.md` (or any `planning-artifacts/` file) and NO `sprint-status.yaml` key or `ready-for-dev` story file, the proposal is in the defect branch above. HALT and surface: "`files_to_change` names `epics.md` but no board-reachable execution artifact. An `epics.md` entry is DESCRIBED (STD-SCOPEREG-001 §4) — actionable for nothing, and INVISIBLE to the board-reading tooling, which derives its epic population from `sprint-status.yaml` and therefore cannot report the omission later. Add the `sprint-status.yaml` key(s) this scope needs." **Never soften this to a warning.** It reads like a pass because a full, well-written epic exists on disk — that appearance of completeness is exactly the failure (FG-2026-07-31-11: six stories described, zero board rows, four days, three owner raisings).
</check>
