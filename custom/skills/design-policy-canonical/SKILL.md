...
5. **Validate against the policy's Hard Failures and Anti-default compositions sections** before delivering. If a truth-class rule applies (it would let a reader believe something false about the data, the money, the state of the work, or who may see what — `_bmad/bmm/workflows/design/shared/brief-binding-contract.md` §2), revise — do not ship. If a style/composition rule applies, revise when you are the one authoring; when you are interpreting a designer's artifact (Claude Design or `design-synthesize`), report it as an advisory note and do not override the designer.
