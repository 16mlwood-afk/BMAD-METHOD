...
- **Latest wave:** design-implement — **gate the §7 frame-coverage axis in the completion report** (`e72c1cb7`). §2f Frame-coverage rows existed in step-03 since `3a88eee3`, but nothing forced their emission — the checklist had no §2f row and step-04 §9 had no mandatory Frame-coverage section. So a run could sweep only the frames already present in the impl, match them, and report "0 deltas / green" while a designed-but-unbuilt frame stayed invisible (inbound-flow `/orders`: 9 §7 frames promised, 5 swept and matched, 4 drawers never built and never surfaced). Root class `silent-partial-implementation` — the axis was authored, its emission un-enforced. Fix mirrors content-lane/capabilities-removed: a "green" report that never enumerated the §7 list is non-conformant and must be re-emitted; the §7 list, not the found-frame set, is the denominator. **Also delivered the previously-stranded design-lane sync** to inbound-flow `main` (PR #2071): the `3a88eee3`/`22406ca7` waves were synced-but-uncommitted, and since `EnterWorktree` branches from HEAD, every worktree run loaded the pre-§2f copy — the root cause (`worktree-sync-drift`) of the `/orders` false-green this diagnosis started from.
- **Fork vs upstream:** see `## Versioning`.
- **Owed / in-flight:** see `## In-Flight Work`. Skill `last_verified_against_fork_commit` = `e72c1cb7`. **`sync bmad` owed to all 13 targets** to distribute `e72c1cb7` (the gate) — inbound-flow has §2f via PR #2071 but not yet the gate. Open follow-up (carried): confirm design-implement §2f treats a `{consumed_frames}` entry (a "consume from sibling brief" §7 row) as out-of-scope-for-this-bundle, not `FRAME NOT DRAWN`.
