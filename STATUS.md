...
- **Latest wave:** design-synthesize + design-handoff — **draw §7 lookup-drawer frames; redirect lookup-drawer handoffs**. Closes the two deferred follow-ups from `3a88eee3`: enumeration (§7) + verification (§2f) had shipped, but design-synthesize never CONSUMED §7 (its `{screens}` came only from frontmatter routes, never the body's Surface Inventory) so the small §13 lookup drawers still shipped thin/inferred; and a lookup-drawer handoff still bounced opaquely. Root class `silent-partial-implementation` (contract complete, the renderer's wire to it missing). Fix: synthesize step-01 §7a turns every §7 frame into a rendered screen (Gate 1f halts on undrawn) + step-04 drawer-over-parent composition; handoff step-01 §2a redirects (not rejects) a lookup-drawer target to its parent-brief frame. Fork `22406ca7`; synced 13/13.
- **Fork vs upstream:** see `## Versioning`.
- **Owed / in-flight:** see `## In-Flight Work`. Skill `last_verified_against_fork_commit` = `22406ca7`. Open follow-up: confirm design-implement §2f treats a `{consumed_frames}` entry (a "consume from sibling brief" §7 row) as out-of-scope-for-this-bundle, not `FRAME NOT DRAWN`.
