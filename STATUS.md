...
- **Owed / in-flight:** see `## In-Flight Work`. Skill `last_verified_against_fork_commit` = `b4ad51cb`. No sync owed (13/13 current). Open follow-ups: (a) adopt-the-overlay pass for the two Bison projects — convert inbound-flow + accounting-tools policies to `inherits: bison-product-family-policy` + residue-only (separate, deliberate per-project rewrites; NOT auto-done); (b) carried — confirm design-implement §2f treats a `{consumed_frames}` entry as out-of-scope-for-this-bundle, not `FRAME NOT DRAWN`.
