...
- **Latest wave:** design-implement — **§2f frame-coverage gets a URL-path denominator** (`56d44fc9`). On a raw Claude Design URL run there is no brief AND no manifest, so §2f's two contract sources (brief §7, synthesize-manifest) were both absent → frame coverage silently no-opped and the §13 expand-in-context lookup drawers a worklist consumes (warehouse / inbound-batch / import-run / accounting-outcome / catalog / supply-source — the "link to records (lookups)") vanished: their shared inner primitives match elsewhere in the impl, so the component grid greens out while the whole drawer ships unbuilt. Root class `contract-dimension-gap` (URL-path twin of §2f), add-not-cede: the bundle declares its own frames (the `<script src>` comments say "… lookups consumed", modules carry per-frame banners, `app.jsx` carries lookup→target maps) — readable evidence the run already traces. Fix: step-01 **URL.3a** captures `{design_frame_inventory}`; step-03 §2f three-source precedence (brief §7 → bundle frame inventory (URL) → manifest (bundle) → needs-human-confirm); step-04 §9 + checklist generalize the disclosure gate beyond brief §7 ("no brief" ≠ "no contract"). Grounded against the user's live Orders.html bundle. **Synced 13/13 — this wave AND the previously-owed `e72c1cb7` gate now distributed.**
- **Prior wave:** design-implement — gate the §7 frame-coverage axis (`e72c1cb7`); the §2f rows existed since `3a88eee3` but emission was un-enforced.
- **Fork vs upstream:** see `## Versioning`.
- **Owed / in-flight:** see `## In-Flight Work`. Skill `last_verified_against_fork_commit` = `56d44fc9`. No sync owed (13/13 current). Open follow-up (carried): confirm design-implement §2f treats a `{consumed_frames}` entry (a "consume from sibling brief" §7 row) as out-of-scope-for-this-bundle, not `FRAME NOT DRAWN`.
